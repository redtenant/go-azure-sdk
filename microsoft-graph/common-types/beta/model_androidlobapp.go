...
var _ json.Unmarshaler = &AndroidLobApp{}

func (s *AndroidLobApp) UnmarshalJSON(bytes []byte) error {
	var decoded struct {
		MinimumSupportedOperatingSystem *AndroidMinimumOperatingSystem `json:"minimumSupportedOperatingSystem,omitempty"`
		PackageId                       nullable.Type[string]          `json:"packageId,omitempty"`
		TargetedPlatforms               *AndroidTargetedPlatforms      `json:"targetedPlatforms,omitempty"`
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling AndroidLobApp into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["relationships"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Relationships' for 'AndroidLobApp': %+v", i, err)
			}
