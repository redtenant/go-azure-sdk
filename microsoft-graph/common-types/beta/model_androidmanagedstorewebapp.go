...
var _ AndroidManagedStoreApp = AndroidManagedStoreWebApp{}

type AndroidManagedStoreWebApp struct {

	// Fields inherited from AndroidManagedStoreApp
...
func (s AndroidManagedStoreWebApp) AndroidManagedStoreApp() BaseAndroidManagedStoreAppImpl {
	return BaseAndroidManagedStoreAppImpl{
		AppIdentifier:         s.AppIdentifier,
		AppStoreUrl:           s.AppStoreUrl,
		AppTracks:             s.AppTracks,
		IsPrivate:             s.IsPrivate,
		IsSystemApp:           s.IsSystemApp,
		PackageId:             s.PackageId,
		SupportsOemConfig:     s.SupportsOemConfig,
		TotalLicenseCount:     s.TotalLicenseCount,
		UsedLicenseCount:      s.UsedLicenseCount,
...
func (s AndroidManagedStoreWebApp) MobileApp() BaseMobileAppImpl {
...
func (s AndroidManagedStoreWebApp) Entity() BaseEntityImpl {
...
var _ json.Marshaler = AndroidManagedStoreWebApp{}

func (s AndroidManagedStoreWebApp) MarshalJSON() ([]byte, error) {
	type wrapper AndroidManagedStoreWebApp
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling AndroidManagedStoreWebApp: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling AndroidManagedStoreWebApp: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.androidManagedStoreWebApp"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling AndroidManagedStoreWebApp: %+v", err)
	}

	return encoded, nil
}

var _ json.Unmarshaler = &AndroidManagedStoreWebApp{}

func (s *AndroidManagedStoreWebApp) UnmarshalJSON(bytes []byte) error {
	var decoded struct {
		AppIdentifier         nullable.Type[string]          `json:"appIdentifier,omitempty"`
		AppStoreUrl           nullable.Type[string]          `json:"appStoreUrl,omitempty"`
		AppTracks             *[]AndroidManagedStoreAppTrack `json:"appTracks,omitempty"`
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling AndroidManagedStoreWebApp into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["relationships"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Relationships' for 'AndroidManagedStoreWebApp': %+v", i, err)
			}
