...
func (s AndroidStoreApp) MobileApp() BaseMobileAppImpl {
...
func (s AndroidStoreApp) Entity() BaseEntityImpl {
...
var _ json.Marshaler = AndroidStoreApp{}

func (s AndroidStoreApp) MarshalJSON() ([]byte, error) {
	type wrapper AndroidStoreApp
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling AndroidStoreApp: %+v", err)
	}
	...
	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.androidStoreApp"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling AndroidStoreApp: %+v", err)
	}

	return encoded, nil
}

var _ json.Unmarshaler = &AndroidStoreApp{}
...
func (s *AndroidStoreApp) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling AndroidStoreApp into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["relationships"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Relationships' for 'AndroidStoreApp': %+v", i, err)
			}
