...
type Application struct {
	...
	// flows, Microsoft Entra ID sends the token to this redirect URI. Microsoft Entra ID also sends the token to this
	// default URI in SAML IdP-initiated single sign-on. The value must match one of the configured redirect URIs for the
	// application.
	...
	// attribute, use one of the following string values: None, SecurityGroup (for security groups and Microsoft Entra
	// roles), All (this gets all security groups, distribution groups, and Microsoft Entra directory roles that the
	...
	// For more information, see How to: Add Terms of service and privacy statement for registered Microsoft Entra apps.
	...
	// the ROPC flow where the application is configured without specifying a redirect URI. In those cases Microsoft Entra
	// ID interprets the application type based on the value of this property.
	...
	// Specifies whether the Native Authentication APIs are enabled for the application. The possible values are: noneand
	...
	// Specifies whether, as part of OAuth 2.0 token requests, Microsoft Entra ID allows POST requests, as opposed to GET
	// requests. The default is false, which specifies that only GET requests are allowed.
	...
	// object properties. As a result, if you change this property, you may need to change other properties first. For more
	// information, see Validation differences for signInAudience.Supports $filter (eq, ne, not).
...
func (s *Application) UnmarshalJSON(bytes []byte) error {
	...
	s.AuthenticationBehaviors = decoded.AuthenticationBehaviors
	s.Certification = decoded.Certification
	s.ConnectorGroup = decoded.ConnectorGroup
