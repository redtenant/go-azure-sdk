package beta

import (
	"github.com/redtenant/go-azure-sdk/sdk/nullable"
)

// Copyright (c) HashiCorp Inc. All rights reserved.
// Licensed under the MIT License. See NOTICE.txt in the project root for license information.

type AppsAndServicesSettings struct {
	// Controls whether users can start trial subscriptions for apps and services in your organization.
	IsAppAndServicesTrialEnabled nullable.Type[bool] `json:"isAppAndServicesTrialEnabled,omitempty"`

	// Controls whether users can access the Microsoft Store.
	IsOfficeStoreEnabled nullable.Type[bool] `json:"isOfficeStoreEnabled,omitempty"`

	// The OData ID of this entity
	ODataId *string `json:"@odata.id,omitempty"`

	// The OData Type of this entity
	ODataType *string `json:"@odata.type,omitempty"`
}
