...
var _ CloudPCBulkAction = CloudPCBulkRestart{}
...
func (s CloudPCBulkRestart) CloudPCBulkAction() BaseCloudPCBulkActionImpl {
...
func (s CloudPCBulkRestart) Entity() BaseEntityImpl {
...
var _ json.Marshaler = CloudPCBulkRestart{}

func (s CloudPCBulkRestart) MarshalJSON() ([]byte, error) {
	type wrapper CloudPCBulkRestart
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling CloudPCBulkRestart: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling CloudPCBulkRestart: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.cloudPcBulkRestart"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling CloudPCBulkRestart: %+v", err)
	}
