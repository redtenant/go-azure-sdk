...
func (s CloudPCBulkRestore) CloudPCBulkAction() BaseCloudPCBulkActionImpl {
...
func (s CloudPCBulkRestore) Entity() BaseEntityImpl {
...
var _ json.Marshaler = CloudPCBulkRestore{}

func (s CloudPCBulkRestore) MarshalJSON() ([]byte, error) {
	type wrapper CloudPCBulkRestore
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling CloudPCBulkRestore: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling CloudPCBulkRestore: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.cloudPcBulkRestore"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling CloudPCBulkRestore: %+v", err)
	}
