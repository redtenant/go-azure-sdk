...
func UnmarshalCommsOperationImplementation(input []byte) (CommsOperation, error) {
	...
	if strings.EqualFold(value, "#microsoft.graph.muteParticipantsOperation") {
		var out MuteParticipantsOperation
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into MuteParticipantsOperation: %+v", err)
		}
		return out, nil
	}
