...
func UnmarshalCustomCalloutExtensionImplementation(input []byte) (CustomCalloutExtension, error) {
	...
	if strings.EqualFold(value, "#microsoft.graph.customAccessPackageWorkflowExtension") {
		var out CustomAccessPackageWorkflowExtension
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into CustomAccessPackageWorkflowExtension: %+v", err)
		}
		return out, nil
	}
