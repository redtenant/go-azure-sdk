...
func UnmarshalDeviceAndAppManagementAssignmentTargetImplementation(input []byte) (DeviceAndAppManagementAssignmentTarget, error) {
	...
	if strings.EqualFold(value, "#microsoft.graph.androidFotaDeploymentAssignmentTarget") {
		var out AndroidFotaDeploymentAssignmentTarget
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into AndroidFotaDeploymentAssignmentTarget: %+v", err)
		}
		return out, nil
	}
