...
func (s GcpUser) GcpIdentity() BaseGcpIdentityImpl {
...
func (s GcpUser) AuthorizationSystemIdentity() BaseAuthorizationSystemIdentityImpl {
...
func (s GcpUser) Entity() BaseEntityImpl {
...
var _ json.Marshaler = GcpUser{}

func (s GcpUser) MarshalJSON() ([]byte, error) {
	type wrapper GcpUser
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling GcpUser: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling GcpUser: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.gcpUser"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling GcpUser: %+v", err)
	}
...
func (s *GcpUser) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling GcpUser into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["authorizationSystem"]; ok {
		impl, err := UnmarshalAuthorizationSystemImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'AuthorizationSystem' for 'GcpUser': %+v", err)
		}
	...
	if v, ok := temp["source"]; ok {
		...
		if err != nil {
			return fmt.Errorf("unmarshaling field 'Source' for 'GcpUser': %+v", err)
		}
		s.Source = impl
	}
