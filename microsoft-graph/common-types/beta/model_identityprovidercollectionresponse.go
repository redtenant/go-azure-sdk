...
func (s *IdentityProviderCollectionResponse) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling IdentityProviderCollectionResponse into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["value"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Value' for 'IdentityProviderCollectionResponse': %+v", i, err)
			}
