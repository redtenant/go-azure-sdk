...
var _ MobileApp = IosiPadOSWebClip{}
...
type IosiPadOSWebClip struct {
	...
	IgnoreManifestScope nullable.Type[bool] `json:"ignoreManifestScope,omitempty"`
	...
	TargetApplicationBundleIdentifier nullable.Type[string] `json:"targetApplicationBundleIdentifier,omitempty"`

	// Whether or not to use managed browser. When TRUE, the app will be required to be opened in Microsoft Edge. When
	// FALSE, the app will not be required to be opened in Microsoft Edge. By default, this property is set to FALSE.
...
func (s IosiPadOSWebClip) MobileApp() BaseMobileAppImpl {
...
func (s IosiPadOSWebClip) Entity() BaseEntityImpl {
...
var _ json.Marshaler = IosiPadOSWebClip{}

func (s IosiPadOSWebClip) MarshalJSON() ([]byte, error) {
	type wrapper IosiPadOSWebClip
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling IosiPadOSWebClip: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling IosiPadOSWebClip: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.iosiPadOSWebClip"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling IosiPadOSWebClip: %+v", err)
	}

	return encoded, nil
}

var _ json.Unmarshaler = &IosiPadOSWebClip{}

func (s *IosiPadOSWebClip) UnmarshalJSON(bytes []byte) error {
	var decoded struct {
		AppUrl                            nullable.Type[string]     `json:"appUrl,omitempty"`
		FullScreenEnabled                 nullable.Type[bool]       `json:"fullScreenEnabled,omitempty"`
		IgnoreManifestScope               nullable.Type[bool]       `json:"ignoreManifestScope,omitempty"`
	...
	s.IgnoreManifestScope = decoded.IgnoreManifestScope
	s.PreComposedIconEnabled = decoded.PreComposedIconEnabled
	s.TargetApplicationBundleIdentifier = decoded.TargetApplicationBundleIdentifier
	s.UseManagedBrowser = decoded.UseManagedBrowser
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling IosiPadOSWebClip into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["relationships"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Relationships' for 'IosiPadOSWebClip': %+v", i, err)
			}
