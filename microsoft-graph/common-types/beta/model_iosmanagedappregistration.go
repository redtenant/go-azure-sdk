...
var _ ManagedAppRegistration = IosManagedAppRegistration{}
...
func (s IosManagedAppRegistration) ManagedAppRegistration() BaseManagedAppRegistrationImpl {
...
func (s IosManagedAppRegistration) Entity() BaseEntityImpl {
...
var _ json.Marshaler = IosManagedAppRegistration{}

func (s IosManagedAppRegistration) MarshalJSON() ([]byte, error) {
	type wrapper IosManagedAppRegistration
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling IosManagedAppRegistration: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling IosManagedAppRegistration: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.iosManagedAppRegistration"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling IosManagedAppRegistration: %+v", err)
	}
...
func (s *IosManagedAppRegistration) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling IosManagedAppRegistration into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["appIdentifier"]; ok {
		impl, err := UnmarshalMobileAppIdentifierImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'AppIdentifier' for 'IosManagedAppRegistration': %+v", err)
		}
	...
	if v, ok := temp["appliedPolicies"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'AppliedPolicies' for 'IosManagedAppRegistration': %+v", i, err)
			}
	...
	if v, ok := temp["intendedPolicies"]; ok {
		...
		for i, val := range listTemp {
			impl, err := UnmarshalManagedAppPolicyImplementation(val)
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'IntendedPolicies' for 'IosManagedAppRegistration': %+v", i, err)
			}
			output = append(output, impl)
		}
