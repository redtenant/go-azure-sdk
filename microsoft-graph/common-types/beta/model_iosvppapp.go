...
var _ json.Unmarshaler = &IosVppApp{}

func (s *IosVppApp) UnmarshalJSON(bytes []byte) error {
	var decoded struct {
		AppStoreUrl                nullable.Type[string]                  `json:"appStoreUrl,omitempty"`
		ApplicableDeviceType       *IosDeviceType                         `json:"applicableDeviceType,omitempty"`
		BundleId                   nullable.Type[string]                  `json:"bundleId,omitempty"`
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling IosVppApp into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["assignedLicenses"]; ok {
		var listTemp []json.RawMessage
		if err := json.Unmarshal(v, &listTemp); err != nil {
			return fmt.Errorf("unmarshaling AssignedLicenses into list []json.RawMessage: %+v", err)
		}
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'AssignedLicenses' for 'IosVppApp': %+v", i, err)
			}
			output = append(output, impl)
		}
		s.AssignedLicenses = &output
	}
	...
	if v, ok := temp["relationships"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Relationships' for 'IosVppApp': %+v", i, err)
			}
			output = append(output, impl)
		}
