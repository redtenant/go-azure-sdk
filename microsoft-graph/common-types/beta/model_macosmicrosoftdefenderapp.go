...
var _ MobileApp = MacOSMicrosoftDefenderApp{}
...
func (s MacOSMicrosoftDefenderApp) MobileApp() BaseMobileAppImpl {
...
func (s MacOSMicrosoftDefenderApp) Entity() BaseEntityImpl {
...
var _ json.Marshaler = MacOSMicrosoftDefenderApp{}

func (s MacOSMicrosoftDefenderApp) MarshalJSON() ([]byte, error) {
	type wrapper MacOSMicrosoftDefenderApp
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling MacOSMicrosoftDefenderApp: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling MacOSMicrosoftDefenderApp: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.macOSMicrosoftDefenderApp"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling MacOSMicrosoftDefenderApp: %+v", err)
	}

	return encoded, nil
}

var _ json.Unmarshaler = &MacOSMicrosoftDefenderApp{}

func (s *MacOSMicrosoftDefenderApp) UnmarshalJSON(bytes []byte) error {
	var decoded struct {
		Assignments           *[]MobileAppAssignment    `json:"assignments,omitempty"`
		Categories            *[]MobileAppCategory      `json:"categories,omitempty"`
		CreatedDateTime       *string                   `json:"createdDateTime,omitempty"`
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling MacOSMicrosoftDefenderApp into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["relationships"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Relationships' for 'MacOSMicrosoftDefenderApp': %+v", i, err)
			}
