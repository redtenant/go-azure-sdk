...
var _ MobileApp = MacOSMicrosoftEdgeApp{}
...
func (s MacOSMicrosoftEdgeApp) MobileApp() BaseMobileAppImpl {
...
func (s MacOSMicrosoftEdgeApp) Entity() BaseEntityImpl {
...
var _ json.Marshaler = MacOSMicrosoftEdgeApp{}

func (s MacOSMicrosoftEdgeApp) MarshalJSON() ([]byte, error) {
	type wrapper MacOSMicrosoftEdgeApp
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling MacOSMicrosoftEdgeApp: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling MacOSMicrosoftEdgeApp: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.macOSMicrosoftEdgeApp"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling MacOSMicrosoftEdgeApp: %+v", err)
	}
...
func (s *MacOSMicrosoftEdgeApp) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling MacOSMicrosoftEdgeApp into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["relationships"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Relationships' for 'MacOSMicrosoftEdgeApp': %+v", i, err)
			}
