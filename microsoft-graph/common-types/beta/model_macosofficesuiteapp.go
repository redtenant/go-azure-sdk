...
var _ MobileApp = MacOSOfficeSuiteApp{}
...
func (s MacOSOfficeSuiteApp) MobileApp() BaseMobileAppImpl {
...
func (s MacOSOfficeSuiteApp) Entity() BaseEntityImpl {
...
var _ json.Marshaler = MacOSOfficeSuiteApp{}

func (s MacOSOfficeSuiteApp) MarshalJSON() ([]byte, error) {
	type wrapper MacOSOfficeSuiteApp
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling MacOSOfficeSuiteApp: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling MacOSOfficeSuiteApp: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.macOSOfficeSuiteApp"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling MacOSOfficeSuiteApp: %+v", err)
	}

	return encoded, nil
}

var _ json.Unmarshaler = &MacOSOfficeSuiteApp{}

func (s *MacOSOfficeSuiteApp) UnmarshalJSON(bytes []byte) error {
	var decoded struct {
		Assignments           *[]MobileAppAssignment    `json:"assignments,omitempty"`
		Categories            *[]MobileAppCategory      `json:"categories,omitempty"`
		CreatedDateTime       *string                   `json:"createdDateTime,omitempty"`
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling MacOSOfficeSuiteApp into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["relationships"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Relationships' for 'MacOSOfficeSuiteApp': %+v", i, err)
			}
