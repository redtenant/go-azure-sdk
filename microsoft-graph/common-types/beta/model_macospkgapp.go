...
var _ MobileLobApp = MacOSPkgApp{}
...
type MacOSPkgApp struct {
	...
	PostInstallScript *MacOSAppScript `json:"postInstallScript,omitempty"`

	// ComplexType macOSAppScript the contains the post-install script for the app. This will execute on the macOS device
	// after the app is installed.
	PreInstallScript *MacOSAppScript `json:"preInstallScript,omitempty"`
...
func (s MacOSPkgApp) MobileLobApp() BaseMobileLobAppImpl {
...
func (s MacOSPkgApp) MobileApp() BaseMobileAppImpl {
...
func (s MacOSPkgApp) Entity() BaseEntityImpl {
...
var _ json.Marshaler = MacOSPkgApp{}

func (s MacOSPkgApp) MarshalJSON() ([]byte, error) {
	type wrapper MacOSPkgApp
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling MacOSPkgApp: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling MacOSPkgApp: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.macOSPkgApp"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling MacOSPkgApp: %+v", err)
	}

	return encoded, nil
}

var _ json.Unmarshaler = &MacOSPkgApp{}

func (s *MacOSPkgApp) UnmarshalJSON(bytes []byte) error {
	var decoded struct {
		IgnoreVersionDetection          *bool                        `json:"ignoreVersionDetection,omitempty"`
		IncludedApps                    *[]MacOSIncludedApp          `json:"includedApps,omitempty"`
		MinimumSupportedOperatingSystem *MacOSMinimumOperatingSystem `json:"minimumSupportedOperatingSystem,omitempty"`
	...
	s.PostInstallScript = decoded.PostInstallScript
	s.PreInstallScript = decoded.PreInstallScript
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling MacOSPkgApp into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["relationships"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Relationships' for 'MacOSPkgApp': %+v", i, err)
			}
