...
func (s MacOsVppApp) MobileApp() BaseMobileAppImpl {
...
func (s MacOsVppApp) Entity() BaseEntityImpl {
...
var _ json.Marshaler = MacOsVppApp{}

func (s MacOsVppApp) MarshalJSON() ([]byte, error) {
	type wrapper MacOsVppApp
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling MacOsVppApp: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling MacOsVppApp: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.macOsVppApp"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling MacOsVppApp: %+v", err)
	}
...
func (s *MacOsVppApp) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling MacOsVppApp into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["relationships"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Relationships' for 'MacOsVppApp': %+v", i, err)
			}
