...
var _ json.Unmarshaler = &BaseManagedAppImpl{}

func (s *BaseManagedAppImpl) UnmarshalJSON(bytes []byte) error {
	var decoded struct {
		AppAvailability       *ManagedAppAvailability   `json:"appAvailability,omitempty"`
		Version               nullable.Type[string]     `json:"version,omitempty"`
		Assignments           *[]MobileAppAssignment    `json:"assignments,omitempty"`
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling BaseManagedAppImpl into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["relationships"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Relationships' for 'BaseManagedAppImpl': %+v", i, err)
			}
