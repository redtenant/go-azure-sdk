...
func UnmarshalManagedAppPolicyImplementation(input []byte) (ManagedAppPolicy, error) {
	...
	if strings.EqualFold(value, "#microsoft.graph.windowsManagedAppProtection") {
		var out WindowsManagedAppProtection
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into WindowsManagedAppProtection: %+v", err)
		}
		return out, nil
	}
