...
func UnmarshalManagedAppRegistrationImplementation(input []byte) (ManagedAppRegistration, error) {
	...
	if strings.EqualFold(value, "#microsoft.graph.windowsManagedAppRegistration") {
		var out WindowsManagedAppRegistration
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into WindowsManagedAppRegistration: %+v", err)
		}
		return out, nil
	}
