...
func UnmarshalManagedDeviceMobileAppConfigurationImplementation(input []byte) (ManagedDeviceMobileAppConfiguration, error) {
	...
	if strings.EqualFold(value, "#microsoft.graph.androidForWorkMobileAppConfiguration") {
		var out AndroidForWorkMobileAppConfiguration
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into AndroidForWorkMobileAppConfiguration: %+v", err)
		}
		return out, nil
	}

	if strings.EqualFold(value, "#microsoft.graph.androidManagedStoreAppConfiguration") {
		var out AndroidManagedStoreAppConfiguration
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into AndroidManagedStoreAppConfiguration: %+v", err)
		}
		return out, nil
	}
