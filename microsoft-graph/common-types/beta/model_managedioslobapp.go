...
var _ json.Unmarshaler = &ManagedIOSLobApp{}

func (s *ManagedIOSLobApp) UnmarshalJSON(bytes []byte) error {
	var decoded struct {
		ApplicableDeviceType            *IosDeviceType             `json:"applicableDeviceType,omitempty"`
		BuildNumber                     nullable.Type[string]      `json:"buildNumber,omitempty"`
		BundleId                        nullable.Type[string]      `json:"bundleId,omitempty"`
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling ManagedIOSLobApp into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["relationships"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Relationships' for 'ManagedIOSLobApp': %+v", i, err)
			}
