...
func UnmarshalNamedLocationImplementation(input []byte) (NamedLocation, error) {
	...
	if strings.EqualFold(value, "#microsoft.graph.compliantNetworkNamedLocation") {
		var out CompliantNetworkNamedLocation
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into CompliantNetworkNamedLocation: %+v", err)
		}
		return out, nil
	}
