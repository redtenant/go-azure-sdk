...
func (s BaseOmaSettingImpl) MarshalJSON() ([]byte, error) {
	type wrapper BaseOmaSettingImpl
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling BaseOmaSettingImpl: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling BaseOmaSettingImpl: %+v", err)
	}

	delete(decoded, "isEncrypted")
	delete(decoded, "secretReferenceValueId")

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling BaseOmaSettingImpl: %+v", err)
	}
