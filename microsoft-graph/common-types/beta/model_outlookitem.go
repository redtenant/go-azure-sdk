...
func UnmarshalOutlookItemImplementation(input []byte) (OutlookItem, error) {
	...
	if strings.EqualFold(value, "#microsoft.graph.note") {
		var out Note
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into Note: %+v", err)
		}
		return out, nil
	}

	if strings.EqualFold(value, "#microsoft.graph.outlookTask") {
		var out OutlookTask
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into OutlookTask: %+v", err)
		}
		return out, nil
	}
