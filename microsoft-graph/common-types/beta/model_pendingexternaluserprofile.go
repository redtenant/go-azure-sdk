...
var _ ExternalProfile = PendingExternalUserProfile{}
...
func (s PendingExternalUserProfile) ExternalProfile() BaseExternalProfileImpl {
...
func (s PendingExternalUserProfile) DirectoryObject() BaseDirectoryObjectImpl {
...
func (s PendingExternalUserProfile) Entity() BaseEntityImpl {
...
var _ json.Marshaler = PendingExternalUserProfile{}

func (s PendingExternalUserProfile) MarshalJSON() ([]byte, error) {
	type wrapper PendingExternalUserProfile
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling PendingExternalUserProfile: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling PendingExternalUserProfile: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.pendingExternalUserProfile"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling PendingExternalUserProfile: %+v", err)
	}
