...
var _ json.Unmarshaler = &PlannerTaskCollectionResponse{}

func (s *PlannerTaskCollectionResponse) UnmarshalJSON(bytes []byte) error {
	var decoded struct {
		ODataId       *string               `json:"@odata.id,omitempty"`
		ODataNextLink nullable.Type[string] `json:"@odata.nextLink,omitempty"`
		ODataType     *string               `json:"@odata.type,omitempty"`
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling PlannerTaskCollectionResponse into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["value"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Value' for 'PlannerTaskCollectionResponse': %+v", i, err)
			}
