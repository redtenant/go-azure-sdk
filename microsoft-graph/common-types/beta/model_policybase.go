...
func UnmarshalPolicyBaseImplementation(input []byte) (PolicyBase, error) {
	...
	if strings.EqualFold(value, "#microsoft.graph.externalIdentitiesPolicy") {
		var out ExternalIdentitiesPolicy
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into ExternalIdentitiesPolicy: %+v", err)
		}
		return out, nil
	}
	...
	if strings.EqualFold(value, "#microsoft.graph.servicePrincipalCreationPolicy") {
		var out ServicePrincipalCreationPolicy
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into ServicePrincipalCreationPolicy: %+v", err)
		}
		return out, nil
	}
	...
	if strings.EqualFold(value, "#microsoft.graph.tenantRelationshipAccessPolicyBase") {
		var out TenantRelationshipAccessPolicyBase
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into TenantRelationshipAccessPolicyBase: %+v", err)
		}
		return out, nil
	}
