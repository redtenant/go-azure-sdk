...
var _ Entity = PurchaseInvoiceLine{}
...
type PurchaseInvoiceLine struct {
	...
	ExpectedReceiptDate       nullable.Type[string]  `json:"expectedReceiptDate,omitempty"`
...
func (s PurchaseInvoiceLine) Entity() BaseEntityImpl {
...
var _ json.Marshaler = PurchaseInvoiceLine{}

func (s PurchaseInvoiceLine) MarshalJSON() ([]byte, error) {
	type wrapper PurchaseInvoiceLine
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling PurchaseInvoiceLine: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling PurchaseInvoiceLine: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.purchaseInvoiceLine"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling PurchaseInvoiceLine: %+v", err)
	}
