package beta

import (
	"encoding/json"
	"fmt"

	"github.com/redtenant/go-azure-sdk/sdk/nullable"
)

// Copyright (c) HashiCorp Inc. All rights reserved.
// Licensed under the MIT License. See NOTICE.txt in the project root for license information.

var _ Entity = RoleScopeTag{}

type RoleScopeTag struct {
	// The list of assignments for this Role Scope Tag.
	Assignments *[]RoleScopeTagAutoAssignment `json:"assignments,omitempty"`

	// Description of the Role Scope Tag.
	Description nullable.Type[string] `json:"description,omitempty"`

	// The display or friendly name of the Role Scope Tag.
	DisplayName nullable.Type[string] `json:"displayName,omitempty"`

	// Description of the Role Scope Tag. This property is read-only.
	IsBuiltIn *bool `json:"isBuiltIn,omitempty"`

	// Fields inherited from Entity

	// The unique identifier for an entity. Read-only.
	Id *string `json:"id,omitempty"`

	// The OData ID of this entity
	ODataId *string `json:"@odata.id,omitempty"`

	// The OData Type of this entity
	ODataType *string `json:"@odata.type,omitempty"`

	// Model Behaviors
	OmitDiscriminatedValue bool `json:"-"`
}

func (s RoleScopeTag) Entity() BaseEntityImpl {
	return BaseEntityImpl{
		Id:        s.Id,
		ODataId:   s.ODataId,
		ODataType: s.ODataType,
	}
}

var _ json.Marshaler = RoleScopeTag{}

func (s RoleScopeTag) MarshalJSON() ([]byte, error) {
	type wrapper RoleScopeTag
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling RoleScopeTag: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling RoleScopeTag: %+v", err)
	}

	delete(decoded, "isBuiltIn")

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.roleScopeTag"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling RoleScopeTag: %+v", err)
	}

	return encoded, nil
}
