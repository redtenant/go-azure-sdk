...
func (s SalesInvoiceLine) Entity() BaseEntityImpl {
...
var _ json.Marshaler = SalesInvoiceLine{}

func (s SalesInvoiceLine) MarshalJSON() ([]byte, error) {
	type wrapper SalesInvoiceLine
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling SalesInvoiceLine: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling SalesInvoiceLine: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.salesInvoiceLine"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling SalesInvoiceLine: %+v", err)
	}
