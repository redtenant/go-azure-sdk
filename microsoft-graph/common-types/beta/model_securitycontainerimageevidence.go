...
var _ SecurityAlertEvidence = SecurityContainerImageEvidence{}
...
type SecurityContainerImageEvidence struct {
	...
	ImageId nullable.Type[string] `json:"imageId,omitempty"`
...
func (s SecurityContainerImageEvidence) SecurityAlertEvidence() BaseSecurityAlertEvidenceImpl {
...
var _ json.Marshaler = SecurityContainerImageEvidence{}

func (s SecurityContainerImageEvidence) MarshalJSON() ([]byte, error) {
	type wrapper SecurityContainerImageEvidence
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling SecurityContainerImageEvidence: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling SecurityContainerImageEvidence: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.security.containerImageEvidence"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling SecurityContainerImageEvidence: %+v", err)
	}
