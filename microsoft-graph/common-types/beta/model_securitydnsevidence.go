...
var _ SecurityAlertEvidence = SecurityDnsEvidence{}

type SecurityDnsEvidence struct {
	DnsServerIp   *SecurityIPEvidence   `json:"dnsServerIp,omitempty"`
	DomainName    nullable.Type[string] `json:"domainName,omitempty"`
	HostIPAddress *SecurityIPEvidence   `json:"hostIpAddress,omitempty"`
	IPAddresses   *[]SecurityIPEvidence `json:"ipAddresses,omitempty"`
...
func (s SecurityDnsEvidence) SecurityAlertEvidence() BaseSecurityAlertEvidenceImpl {
...
var _ json.Marshaler = SecurityDnsEvidence{}

func (s SecurityDnsEvidence) MarshalJSON() ([]byte, error) {
	type wrapper SecurityDnsEvidence
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling SecurityDnsEvidence: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling SecurityDnsEvidence: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.security.dnsEvidence"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling SecurityDnsEvidence: %+v", err)
	}
