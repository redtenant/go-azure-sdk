...
var _ SecurityCaseOperation = SecurityEdiscoveryIndexOperation{}
...
func (s SecurityEdiscoveryIndexOperation) SecurityCaseOperation() BaseSecurityCaseOperationImpl {
...
func (s SecurityEdiscoveryIndexOperation) Entity() BaseEntityImpl {
...
var _ json.Marshaler = SecurityEdiscoveryIndexOperation{}

func (s SecurityEdiscoveryIndexOperation) MarshalJSON() ([]byte, error) {
	type wrapper SecurityEdiscoveryIndexOperation
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling SecurityEdiscoveryIndexOperation: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling SecurityEdiscoveryIndexOperation: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.security.ediscoveryIndexOperation"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling SecurityEdiscoveryIndexOperation: %+v", err)
	}

	return encoded, nil
}

var _ json.Unmarshaler = &SecurityEdiscoveryIndexOperation{}

func (s *SecurityEdiscoveryIndexOperation) UnmarshalJSON(bytes []byte) error {
	var decoded struct {
		Action            *SecurityCaseAction          `json:"action,omitempty"`
		CompletedDateTime nullable.Type[string]        `json:"completedDateTime,omitempty"`
		CreatedDateTime   nullable.Type[string]        `json:"createdDateTime,omitempty"`
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling SecurityEdiscoveryIndexOperation into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["createdBy"]; ok {
		impl, err := UnmarshalIdentitySetImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'CreatedBy' for 'SecurityEdiscoveryIndexOperation': %+v", err)
		}
		s.CreatedBy = impl
	}
