...
var _ SecurityAlertEvidence = SecurityGitHubRepoEvidence{}

type SecurityGitHubRepoEvidence struct {
	BaseUrl   nullable.Type[string] `json:"baseUrl,omitempty"`
	...
	RepoId    nullable.Type[string] `json:"repoId,omitempty"`
...
func (s SecurityGitHubRepoEvidence) SecurityAlertEvidence() BaseSecurityAlertEvidenceImpl {
...
var _ json.Marshaler = SecurityGitHubRepoEvidence{}

func (s SecurityGitHubRepoEvidence) MarshalJSON() ([]byte, error) {
	type wrapper SecurityGitHubRepoEvidence
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling SecurityGitHubRepoEvidence: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling SecurityGitHubRepoEvidence: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.security.gitHubRepoEvidence"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling SecurityGitHubRepoEvidence: %+v", err)
	}
