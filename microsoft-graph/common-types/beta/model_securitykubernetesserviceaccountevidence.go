...
var _ SecurityAlertEvidence = SecurityKubernetesServiceAccountEvidence{}
...
func (s SecurityKubernetesServiceAccountEvidence) SecurityAlertEvidence() BaseSecurityAlertEvidenceImpl {
...
var _ json.Marshaler = SecurityKubernetesServiceAccountEvidence{}

func (s SecurityKubernetesServiceAccountEvidence) MarshalJSON() ([]byte, error) {
	type wrapper SecurityKubernetesServiceAccountEvidence
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling SecurityKubernetesServiceAccountEvidence: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling SecurityKubernetesServiceAccountEvidence: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.security.kubernetesServiceAccountEvidence"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling SecurityKubernetesServiceAccountEvidence: %+v", err)
	}
