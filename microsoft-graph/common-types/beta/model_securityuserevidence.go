...
var _ SecurityAlertEvidence = SecurityUserEvidence{}
...
func (s SecurityUserEvidence) SecurityAlertEvidence() BaseSecurityAlertEvidenceImpl {
...
var _ json.Marshaler = SecurityUserEvidence{}

func (s SecurityUserEvidence) MarshalJSON() ([]byte, error) {
	type wrapper SecurityUserEvidence
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling SecurityUserEvidence: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling SecurityUserEvidence: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.security.userEvidence"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling SecurityUserEvidence: %+v", err)
	}
