...
var _ UnifiedRoleManagementAlertConfiguration = SequentialActivationRenewalsAlertConfiguration{}
...
type SequentialActivationRenewalsAlertConfiguration struct {
	...
	SequentialActivationCounterThreshold nullable.Type[int64] `json:"sequentialActivationCounterThreshold,omitempty"`

	// Time interval between activations to trigger an alert.
	TimeIntervalBetweenActivations nullable.Type[string] `json:"timeIntervalBetweenActivations,omitempty"`
...
func (s SequentialActivationRenewalsAlertConfiguration) UnifiedRoleManagementAlertConfiguration() BaseUnifiedRoleManagementAlertConfigurationImpl {
...
func (s SequentialActivationRenewalsAlertConfiguration) Entity() BaseEntityImpl {
...
var _ json.Marshaler = SequentialActivationRenewalsAlertConfiguration{}

func (s SequentialActivationRenewalsAlertConfiguration) MarshalJSON() ([]byte, error) {
	type wrapper SequentialActivationRenewalsAlertConfiguration
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling SequentialActivationRenewalsAlertConfiguration: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling SequentialActivationRenewalsAlertConfiguration: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.sequentialActivationRenewalsAlertConfiguration"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling SequentialActivationRenewalsAlertConfiguration: %+v", err)
	}
