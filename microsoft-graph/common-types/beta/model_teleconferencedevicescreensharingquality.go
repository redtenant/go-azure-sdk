...
var _ TeleconferenceDeviceVideoQuality = TeleconferenceDeviceScreenSharingQuality{}
...
func (s TeleconferenceDeviceScreenSharingQuality) TeleconferenceDeviceVideoQuality() BaseTeleconferenceDeviceVideoQualityImpl {
	return BaseTeleconferenceDeviceVideoQualityImpl{
...
func (s TeleconferenceDeviceScreenSharingQuality) TeleconferenceDeviceMediaQuality() BaseTeleconferenceDeviceMediaQualityImpl {
...
var _ json.Marshaler = TeleconferenceDeviceScreenSharingQuality{}

func (s TeleconferenceDeviceScreenSharingQuality) MarshalJSON() ([]byte, error) {
	type wrapper TeleconferenceDeviceScreenSharingQuality
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling TeleconferenceDeviceScreenSharingQuality: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling TeleconferenceDeviceScreenSharingQuality: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.teleconferenceDeviceScreenSharingQuality"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling TeleconferenceDeviceScreenSharingQuality: %+v", err)
	}
