...
type UnifiedRoleEligibilityScheduleRequest struct {
	...
	// extend expiring role eligibilities.AdminRenew: For administrators to renew expired eligibilities.UserAdd: For users
	// to activate their eligible assignments.UserRemove: For users to deactivate their active eligible
	// assignments.UserExtend: For users to request to extend their expiring eligible assignments.UserRenew: For users to
	// request to renew their expired eligible assignments.
...
func (s *UnifiedRoleEligibilityScheduleRequest) UnmarshalJSON(bytes []byte) error {
	...
	if v, ok := temp["appScope"]; ok {
		...
		if err != nil {
			return fmt.Errorf("unmarshaling field 'AppScope' for 'UnifiedRoleEligibilityScheduleRequest': %+v", err)
		}
		s.AppScope = &impl
	}
