...
func (s WebApp) MobileApp() BaseMobileAppImpl {
...
func (s WebApp) Entity() BaseEntityImpl {
...
func (s WebApp) MarshalJSON() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("marshaling WebApp: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling WebApp: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.webApp"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling WebApp: %+v", err)
	}

	return encoded, nil
}

var _ json.Unmarshaler = &WebApp{}

func (s *WebApp) UnmarshalJSON(bytes []byte) error {
	var decoded struct {
		AppUrl                nullable.Type[string]     `json:"appUrl,omitempty"`
		UseManagedBrowser     *bool                     `json:"useManagedBrowser,omitempty"`
		Assignments           *[]MobileAppAssignment    `json:"assignments,omitempty"`
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling WebApp into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["relationships"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Relationships' for 'WebApp': %+v", i, err)
			}
