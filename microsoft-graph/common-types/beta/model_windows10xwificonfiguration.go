...
var _ DeviceManagementResourceAccessProfileBase = Windows10XWifiConfiguration{}
...
func (s Windows10XWifiConfiguration) DeviceManagementResourceAccessProfileBase() BaseDeviceManagementResourceAccessProfileBaseImpl {
...
func (s Windows10XWifiConfiguration) Entity() BaseEntityImpl {
...
var _ json.Marshaler = Windows10XWifiConfiguration{}

func (s Windows10XWifiConfiguration) MarshalJSON() ([]byte, error) {
	type wrapper Windows10XWifiConfiguration
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling Windows10XWifiConfiguration: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling Windows10XWifiConfiguration: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.windows10XWifiConfiguration"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling Windows10XWifiConfiguration: %+v", err)
	}
