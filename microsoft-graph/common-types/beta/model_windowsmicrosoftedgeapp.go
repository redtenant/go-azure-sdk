...
var _ MobileApp = WindowsMicrosoftEdgeApp{}
...
type WindowsMicrosoftEdgeApp struct {
	...
	DisplayLanguageLocale nullable.Type[string] `json:"displayLanguageLocale,omitempty"`
...
func (s WindowsMicrosoftEdgeApp) MobileApp() BaseMobileAppImpl {
...
func (s WindowsMicrosoftEdgeApp) Entity() BaseEntityImpl {
...
var _ json.Marshaler = WindowsMicrosoftEdgeApp{}

func (s WindowsMicrosoftEdgeApp) MarshalJSON() ([]byte, error) {
	type wrapper WindowsMicrosoftEdgeApp
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling WindowsMicrosoftEdgeApp: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling WindowsMicrosoftEdgeApp: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.windowsMicrosoftEdgeApp"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling WindowsMicrosoftEdgeApp: %+v", err)
	}

	return encoded, nil
}

var _ json.Unmarshaler = &WindowsMicrosoftEdgeApp{}

func (s *WindowsMicrosoftEdgeApp) UnmarshalJSON(bytes []byte) error {
	var decoded struct {
		Channel               *MicrosoftEdgeChannel     `json:"channel,omitempty"`
		DisplayLanguageLocale nullable.Type[string]     `json:"displayLanguageLocale,omitempty"`
		Assignments           *[]MobileAppAssignment    `json:"assignments,omitempty"`
	...
	s.DisplayLanguageLocale = decoded.DisplayLanguageLocale
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling WindowsMicrosoftEdgeApp into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["relationships"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Relationships' for 'WindowsMicrosoftEdgeApp': %+v", i, err)
			}
