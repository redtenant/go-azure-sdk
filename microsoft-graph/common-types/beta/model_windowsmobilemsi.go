...
var _ MobileLobApp = WindowsMobileMSI{}
...
type WindowsMobileMSI struct {
	...
	// false, app will be installed per-user. If null, service will use the MSI package's default install context. In case
	// of dual-mode MSI, this default will be per-user. Cannot be set for non-dual-mode apps. Cannot be changed after
	// initial creation of the application.
	UseDeviceContext nullable.Type[bool] `json:"useDeviceContext,omitempty"`
...
func (s WindowsMobileMSI) MobileLobApp() BaseMobileLobAppImpl {
...
func (s WindowsMobileMSI) MobileApp() BaseMobileAppImpl {
...
func (s WindowsMobileMSI) Entity() BaseEntityImpl {
...
var _ json.Marshaler = WindowsMobileMSI{}

func (s WindowsMobileMSI) MarshalJSON() ([]byte, error) {
	type wrapper WindowsMobileMSI
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling WindowsMobileMSI: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling WindowsMobileMSI: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.windowsMobileMSI"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling WindowsMobileMSI: %+v", err)
	}

	return encoded, nil
}

var _ json.Unmarshaler = &WindowsMobileMSI{}

func (s *WindowsMobileMSI) UnmarshalJSON(bytes []byte) error {
	var decoded struct {
		CommandLine             nullable.Type[string]     `json:"commandLine,omitempty"`
		IdentityVersion         nullable.Type[string]     `json:"identityVersion,omitempty"`
		IgnoreVersionDetection  *bool                     `json:"ignoreVersionDetection,omitempty"`
	...
	s.CommandLine = decoded.CommandLine
	...
	s.ProductVersion = decoded.ProductVersion
	s.UseDeviceContext = decoded.UseDeviceContext
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling WindowsMobileMSI into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["relationships"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Relationships' for 'WindowsMobileMSI': %+v", i, err)
			}
