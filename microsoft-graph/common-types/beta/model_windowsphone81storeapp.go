...
var _ MobileApp = WindowsPhone81StoreApp{}
...
func (s WindowsPhone81StoreApp) MobileApp() BaseMobileAppImpl {
...
func (s WindowsPhone81StoreApp) Entity() BaseEntityImpl {
...
var _ json.Marshaler = WindowsPhone81StoreApp{}

func (s WindowsPhone81StoreApp) MarshalJSON() ([]byte, error) {
	type wrapper WindowsPhone81StoreApp
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling WindowsPhone81StoreApp: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling WindowsPhone81StoreApp: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.windowsPhone81StoreApp"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling WindowsPhone81StoreApp: %+v", err)
	}

	return encoded, nil
}

var _ json.Unmarshaler = &WindowsPhone81StoreApp{}

func (s *WindowsPhone81StoreApp) UnmarshalJSON(bytes []byte) error {
	var decoded struct {
		AppStoreUrl           nullable.Type[string]     `json:"appStoreUrl,omitempty"`
		Assignments           *[]MobileAppAssignment    `json:"assignments,omitempty"`
		Categories            *[]MobileAppCategory      `json:"categories,omitempty"`
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling WindowsPhone81StoreApp into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["relationships"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Relationships' for 'WindowsPhone81StoreApp': %+v", i, err)
			}
