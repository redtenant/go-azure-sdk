...
var _ MobileLobApp = WindowsPhoneXAP{}
...
func (s WindowsPhoneXAP) MobileLobApp() BaseMobileLobAppImpl {
...
func (s WindowsPhoneXAP) MobileApp() BaseMobileAppImpl {
...
func (s WindowsPhoneXAP) Entity() BaseEntityImpl {
...
var _ json.Marshaler = WindowsPhoneXAP{}

func (s WindowsPhoneXAP) MarshalJSON() ([]byte, error) {
	type wrapper WindowsPhoneXAP
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling WindowsPhoneXAP: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling WindowsPhoneXAP: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.windowsPhoneXAP"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling WindowsPhoneXAP: %+v", err)
	}

	return encoded, nil
}

var _ json.Unmarshaler = &WindowsPhoneXAP{}

func (s *WindowsPhoneXAP) UnmarshalJSON(bytes []byte) error {
	var decoded struct {
		IdentityVersion                 nullable.Type[string]          `json:"identityVersion,omitempty"`
		MinimumSupportedOperatingSystem *WindowsMinimumOperatingSystem `json:"minimumSupportedOperatingSystem,omitempty"`
		ProductIdentifier               *string                        `json:"productIdentifier,omitempty"`
	...
	s.ProductIdentifier = decoded.ProductIdentifier
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling WindowsPhoneXAP into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["relationships"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Relationships' for 'WindowsPhoneXAP': %+v", i, err)
			}
