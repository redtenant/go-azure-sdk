...
var _ MobileApp = WindowsStoreApp{}
...
func (s WindowsStoreApp) MobileApp() BaseMobileAppImpl {
...
func (s WindowsStoreApp) Entity() BaseEntityImpl {
...
var _ json.Marshaler = WindowsStoreApp{}

func (s WindowsStoreApp) MarshalJSON() ([]byte, error) {
	type wrapper WindowsStoreApp
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling WindowsStoreApp: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling WindowsStoreApp: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.windowsStoreApp"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling WindowsStoreApp: %+v", err)
	}

	return encoded, nil
}

var _ json.Unmarshaler = &WindowsStoreApp{}

func (s *WindowsStoreApp) UnmarshalJSON(bytes []byte) error {
	var decoded struct {
		AppStoreUrl           nullable.Type[string]     `json:"appStoreUrl,omitempty"`
		Assignments           *[]MobileAppAssignment    `json:"assignments,omitempty"`
		Categories            *[]MobileAppCategory      `json:"categories,omitempty"`
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling WindowsStoreApp into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["relationships"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Relationships' for 'WindowsStoreApp': %+v", i, err)
			}
