...
type WinGetApp struct {
	...
	// repository.
	ManifestHash nullable.Type[string] `json:"manifestHash,omitempty"`

	// The PackageIdentifier from the WinGet source repository REST API. This also maps to the Id when using the WinGet
	// client command line application. Required at creation time, cannot be modified on existing objects.
	PackageIdentifier nullable.Type[string] `json:"packageIdentifier,omitempty"`
...
func (s WinGetApp) MobileApp() BaseMobileAppImpl {
...
func (s WinGetApp) Entity() BaseEntityImpl {
...
var _ json.Marshaler = WinGetApp{}

func (s WinGetApp) MarshalJSON() ([]byte, error) {
	type wrapper WinGetApp
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling WinGetApp: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling WinGetApp: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.winGetApp"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling WinGetApp: %+v", err)
	}

	return encoded, nil
}

var _ json.Unmarshaler = &WinGetApp{}

func (s *WinGetApp) UnmarshalJSON(bytes []byte) error {
	var decoded struct {
		InstallExperience     *WinGetAppInstallExperience `json:"installExperience,omitempty"`
		ManifestHash          nullable.Type[string]       `json:"manifestHash,omitempty"`
		PackageIdentifier     nullable.Type[string]       `json:"packageIdentifier,omitempty"`
	...
	s.ManifestHash = decoded.ManifestHash
	s.PackageIdentifier = decoded.PackageIdentifier
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling WinGetApp into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["relationships"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Relationships' for 'WinGetApp': %+v", i, err)
			}
