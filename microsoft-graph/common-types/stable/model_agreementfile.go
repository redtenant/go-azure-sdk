...
var _ AgreementFileProperties = AgreementFile{}
...
func (s AgreementFile) AgreementFileProperties() BaseAgreementFilePropertiesImpl {
...
func (s AgreementFile) Entity() BaseEntityImpl {
...
var _ json.Marshaler = AgreementFile{}

func (s AgreementFile) MarshalJSON() ([]byte, error) {
	type wrapper AgreementFile
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling AgreementFile: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling AgreementFile: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.agreementFile"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling AgreementFile: %+v", err)
	}
