...
func UnmarshalDirectoryObjectImplementation(input []byte) (DirectoryObject, error) {
	...
	if strings.EqualFold(value, "#microsoft.graph.groupSettingTemplate") {
		var out GroupSettingTemplate
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into GroupSettingTemplate: %+v", err)
		}
		return out, nil
	}
