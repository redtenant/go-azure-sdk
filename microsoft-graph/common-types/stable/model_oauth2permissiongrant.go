...
type OAuth2PermissionGrant struct {
	// The object id (not appId) of the client service principal for the application that's authorized to act on behalf of a
	...
	// specific user. Consent on behalf of all users can be granted by an administrator. Nonadmin users might be authorized
	// to consent on behalf of themselves in some cases, for some delegated permissions. Required. Supports $filter (eq
	// only).
	...
	// the resource service principal. Must not exceed 3,850 characters in length.
