...
type OrgContact struct {
	...
	// Name of the company that this organizational contact belongs to. Supports $filter (eq, ne, not, ge, le, in,
	...
	// each type can ever be present in the collection.
	...
	// or link from an organizational contact object . Supports $filter (eq, not, for isResolved and serviceInstance).
...
func (s OrgContact) MarshalJSON() ([]byte, error) {
	...
	delete(decoded, "transitiveMemberOf")
