...
var _ SecurityAlertEvidence = SecurityAzureResourceEvidence{}
...
func (s SecurityAzureResourceEvidence) SecurityAlertEvidence() BaseSecurityAlertEvidenceImpl {
...
var _ json.Marshaler = SecurityAzureResourceEvidence{}

func (s SecurityAzureResourceEvidence) MarshalJSON() ([]byte, error) {
	type wrapper SecurityAzureResourceEvidence
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling SecurityAzureResourceEvidence: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling SecurityAzureResourceEvidence: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.security.azureResourceEvidence"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling SecurityAzureResourceEvidence: %+v", err)
	}
