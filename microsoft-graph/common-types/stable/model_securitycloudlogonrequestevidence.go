...
var _ SecurityAlertEvidence = SecurityCloudLogonRequestEvidence{}

type SecurityCloudLogonRequestEvidence struct {
...
func (s SecurityCloudLogonRequestEvidence) SecurityAlertEvidence() BaseSecurityAlertEvidenceImpl {
...
var _ json.Marshaler = SecurityCloudLogonRequestEvidence{}

func (s SecurityCloudLogonRequestEvidence) MarshalJSON() ([]byte, error) {
	type wrapper SecurityCloudLogonRequestEvidence
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling SecurityCloudLogonRequestEvidence: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling SecurityCloudLogonRequestEvidence: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.security.cloudLogonRequestEvidence"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling SecurityCloudLogonRequestEvidence: %+v", err)
	}
