...
var _ SecurityAlertEvidence = SecurityContainerRegistryEvidence{}

type SecurityContainerRegistryEvidence struct {
	// The registry URI.
	Registry nullable.Type[string] `json:"registry,omitempty"`
...
func (s SecurityContainerRegistryEvidence) SecurityAlertEvidence() BaseSecurityAlertEvidenceImpl {
...
var _ json.Marshaler = SecurityContainerRegistryEvidence{}

func (s SecurityContainerRegistryEvidence) MarshalJSON() ([]byte, error) {
	type wrapper SecurityContainerRegistryEvidence
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling SecurityContainerRegistryEvidence: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling SecurityContainerRegistryEvidence: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.security.containerRegistryEvidence"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling SecurityContainerRegistryEvidence: %+v", err)
	}
