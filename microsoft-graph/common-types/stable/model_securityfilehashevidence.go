...
var _ SecurityAlertEvidence = SecurityFileHashEvidence{}

type SecurityFileHashEvidence struct {
	Algorithm *SecurityFileHashAlgorithm `json:"algorithm,omitempty"`
...
func (s SecurityFileHashEvidence) SecurityAlertEvidence() BaseSecurityAlertEvidenceImpl {
...
var _ json.Marshaler = SecurityFileHashEvidence{}

func (s SecurityFileHashEvidence) MarshalJSON() ([]byte, error) {
	type wrapper SecurityFileHashEvidence
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling SecurityFileHashEvidence: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling SecurityFileHashEvidence: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.security.fileHashEvidence"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling SecurityFileHashEvidence: %+v", err)
	}
