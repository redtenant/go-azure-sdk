...
var _ SecurityAlertEvidence = SecurityGitHubOrganizationEvidence{}
...
type SecurityGitHubOrganizationEvidence struct {
	...
	Login       nullable.Type[string] `json:"login,omitempty"`
	OrgId       nullable.Type[string] `json:"orgId,omitempty"`
...
func (s SecurityGitHubOrganizationEvidence) SecurityAlertEvidence() BaseSecurityAlertEvidenceImpl {
...
var _ json.Marshaler = SecurityGitHubOrganizationEvidence{}

func (s SecurityGitHubOrganizationEvidence) MarshalJSON() ([]byte, error) {
	type wrapper SecurityGitHubOrganizationEvidence
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling SecurityGitHubOrganizationEvidence: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling SecurityGitHubOrganizationEvidence: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.security.gitHubOrganizationEvidence"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling SecurityGitHubOrganizationEvidence: %+v", err)
	}
