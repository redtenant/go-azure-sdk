...
var _ SecurityAlertEvidence = SecurityGitHubUserEvidence{}
...
func (s SecurityGitHubUserEvidence) SecurityAlertEvidence() BaseSecurityAlertEvidenceImpl {
...
var _ json.Marshaler = SecurityGitHubUserEvidence{}

func (s SecurityGitHubUserEvidence) MarshalJSON() ([]byte, error) {
	type wrapper SecurityGitHubUserEvidence
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling SecurityGitHubUserEvidence: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling SecurityGitHubUserEvidence: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.security.gitHubUserEvidence"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling SecurityGitHubUserEvidence: %+v", err)
	}
