...
var _ SecurityAlertEvidence = SecurityKubernetesNamespaceEvidence{}
...
func (s SecurityKubernetesNamespaceEvidence) SecurityAlertEvidence() BaseSecurityAlertEvidenceImpl {
...
var _ json.Marshaler = SecurityKubernetesNamespaceEvidence{}

func (s SecurityKubernetesNamespaceEvidence) MarshalJSON() ([]byte, error) {
	type wrapper SecurityKubernetesNamespaceEvidence
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling SecurityKubernetesNamespaceEvidence: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling SecurityKubernetesNamespaceEvidence: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.security.kubernetesNamespaceEvidence"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling SecurityKubernetesNamespaceEvidence: %+v", err)
	}
