...
var _ SecurityAlertEvidence = SecurityMailClusterEvidence{}

type SecurityMailClusterEvidence struct {
	// The clustering logic of the emails inside the cluster.
	ClusterBy nullable.Type[string] `json:"clusterBy,omitempty"`

	// The value utilized to cluster the similar emails.
	ClusterByValue nullable.Type[string] `json:"clusterByValue,omitempty"`

	// Count of emails in the email cluster.
	EmailCount nullable.Type[int64] `json:"emailCount,omitempty"`
...
func (s SecurityMailClusterEvidence) SecurityAlertEvidence() BaseSecurityAlertEvidenceImpl {
...
var _ json.Marshaler = SecurityMailClusterEvidence{}

func (s SecurityMailClusterEvidence) MarshalJSON() ([]byte, error) {
	type wrapper SecurityMailClusterEvidence
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling SecurityMailClusterEvidence: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling SecurityMailClusterEvidence: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.security.mailClusterEvidence"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling SecurityMailClusterEvidence: %+v", err)
	}
