...
var _ SecurityAlertEvidence = SecuritySecurityGroupEvidence{}
...
func (s SecuritySecurityGroupEvidence) SecurityAlertEvidence() BaseSecurityAlertEvidenceImpl {
...
var _ json.Marshaler = SecuritySecurityGroupEvidence{}

func (s SecuritySecurityGroupEvidence) MarshalJSON() ([]byte, error) {
	type wrapper SecuritySecurityGroupEvidence
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling SecuritySecurityGroupEvidence: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling SecuritySecurityGroupEvidence: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.security.securityGroupEvidence"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling SecuritySecurityGroupEvidence: %+v", err)
	}
