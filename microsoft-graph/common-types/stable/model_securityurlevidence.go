...
var _ SecurityAlertEvidence = SecurityUrlEvidence{}
...
func (s SecurityUrlEvidence) SecurityAlertEvidence() BaseSecurityAlertEvidenceImpl {
...
var _ json.Marshaler = SecurityUrlEvidence{}

func (s SecurityUrlEvidence) MarshalJSON() ([]byte, error) {
	type wrapper SecurityUrlEvidence
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling SecurityUrlEvidence: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling SecurityUrlEvidence: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.security.urlEvidence"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling SecurityUrlEvidence: %+v", err)
	}
