...
func (c DeviceConfigurationClient) ListDeviceConfigurations(ctx context.Context, options ListDeviceConfigurationsOperationOptions) (result ListDeviceConfigurationsOperationResponse, err error) {
	...
	temp := make([]beta.DeviceConfiguration, 0)
	if values.Values != nil {
		for i, v := range *values.Values {
			val, err := beta.UnmarshalDeviceConfigurationImplementation(v)
			if err != nil {
				err = fmt.Errorf("unmarshalling item %d for beta.DeviceConfiguration (%q): %+v", i, v, err)
				return result, err
			}
