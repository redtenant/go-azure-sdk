package userexperienceanalyticsapphealthdeviceperformancedetail

// Copyright (c) HashiCorp Inc. All rights reserved.
// Licensed under the MIT License. See NOTICE.txt in the project root for license information.

import "github.com/redtenant/go-azure-sdk/microsoft-graph/common-types/stable"

type UserExperienceAnalyticsAppHealthDevicePerformanceDetailsOperationPredicate struct {
}

func (p UserExperienceAnalyticsAppHealthDevicePerformanceDetailsOperationPredicate) Matches(input stable.UserExperienceAnalyticsAppHealthDevicePerformanceDetails) bool {

	return true
}
