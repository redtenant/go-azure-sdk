...
func DefaultListFeatureRolloutPolicyAppliesTosOperationOptions() ListFeatureRolloutPolicyAppliesTosOperationOptions {
	return ListFeatureRolloutPolicyAppliesTosOperationOptions{}
}
...
// ListFeatureRolloutPolicyAppliesTos - Get appliesTo from directory. Nullable. Specifies a list of directoryObject
// resources that feature is enabled for.
func (c FeatureRolloutPolicyAppliesToClient) ListFeatureRolloutPolicyAppliesTos(ctx context.Context, id beta.DirectoryFeatureRolloutPolicyId, options ListFeatureRolloutPolicyAppliesTosOperationOptions) (result ListFeatureRolloutPolicyAppliesTosOperationResponse, err error) {
...
func (c FeatureRolloutPolicyAppliesToClient) ListFeatureRolloutPolicyAppliesTosComplete(ctx context.Context, id beta.DirectoryFeatureRolloutPolicyId, options ListFeatureRolloutPolicyAppliesTosOperationOptions) (ListFeatureRolloutPolicyAppliesTosCompleteResult, error) {
	return c.ListFeatureRolloutPolicyAppliesTosCompleteMatchingPredicate(ctx, id, options, DirectoryObjectOperationPredicate{})
}

// ListFeatureRolloutPolicyAppliesTosCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c FeatureRolloutPolicyAppliesToClient) ListFeatureRolloutPolicyAppliesTosCompleteMatchingPredicate(ctx context.Context, id beta.DirectoryFeatureRolloutPolicyId, options ListFeatureRolloutPolicyAppliesTosOperationOptions, predicate DirectoryObjectOperationPredicate) (result ListFeatureRolloutPolicyAppliesTosCompleteResult, err error) {
	items := make([]beta.DirectoryObject, 0)

	resp, err := c.ListFeatureRolloutPolicyAppliesTos(ctx, id, options)
	if err != nil {
