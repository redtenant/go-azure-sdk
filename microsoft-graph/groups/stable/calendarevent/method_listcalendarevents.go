...
func DefaultListCalendarEventsOperationOptions() ListCalendarEventsOperationOptions {
	return ListCalendarEventsOperationOptions{}
}
...
func (c CalendarEventClient) ListCalendarEvents(ctx context.Context, id stable.GroupId, options ListCalendarEventsOperationOptions) (result ListCalendarEventsOperationResponse, err error) {
	opts := client.RequestOptions{
		...
		Pager:         &ListCalendarEventsCustomPager{},
...
func (c CalendarEventClient) ListCalendarEventsComplete(ctx context.Context, id stable.GroupId, options ListCalendarEventsOperationOptions) (ListCalendarEventsCompleteResult, error) {
	return c.ListCalendarEventsCompleteMatchingPredicate(ctx, id, options, EventOperationPredicate{})
}

// ListCalendarEventsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c CalendarEventClient) ListCalendarEventsCompleteMatchingPredicate(ctx context.Context, id stable.GroupId, options ListCalendarEventsOperationOptions, predicate EventOperationPredicate) (result ListCalendarEventsCompleteResult, err error) {
	items := make([]stable.Event, 0)

	resp, err := c.ListCalendarEvents(ctx, id, options)
	if err != nil {
	...
	result = ListCalendarEventsCompleteResult{
