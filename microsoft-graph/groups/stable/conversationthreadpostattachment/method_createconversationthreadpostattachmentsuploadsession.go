...
func DefaultCreateConversationThreadPostAttachmentsUploadSessionOperationOptions() CreateConversationThreadPostAttachmentsUploadSessionOperationOptions {
	return CreateConversationThreadPostAttachmentsUploadSessionOperationOptions{}
}
...
// CreateConversationThreadPostAttachmentsUploadSession - Invoke action createUploadSession. Create an upload session
// that allows an app to iteratively upload ranges of a file, so as to attach the file to the specified Outlook item.
// The item can be a message or event. Use this approach to attach a file if the file size is between 3 MB and 150 MB.
// To attach a file that's smaller than 3 MB, do a POST operation on the attachments navigation property of the Outlook
...
func (c ConversationThreadPostAttachmentClient) CreateConversationThreadPostAttachmentsUploadSession(ctx context.Context, id stable.GroupIdConversationIdThreadIdPostId, input CreateConversationThreadPostAttachmentsUploadSessionRequest, options CreateConversationThreadPostAttachmentsUploadSessionOperationOptions) (result CreateConversationThreadPostAttachmentsUploadSessionOperationResponse, err error) {
