...
func DefaultCheckoutDriveRootOperationOptions() CheckoutDriveRootOperationOptions {
	return CheckoutDriveRootOperationOptions{}
}
...
// CheckoutDriveRoot - Invoke action checkout. Check out a driveItem resource to prevent others from editing the
// document, and prevent your changes from being visible until the documented is checked in.
func (c DriveRootClient) CheckoutDriveRoot(ctx context.Context, id stable.GroupIdDriveId, options CheckoutDriveRootOperationOptions) (result CheckoutDriveRootOperationResponse, err error) {
	opts := client.RequestOptions{
		...
		Path:          fmt.Sprintf("%s/root/checkout", id.ID()),
