...
func DefaultListTeamChannelsOperationOptions() ListTeamChannelsOperationOptions {
	return ListTeamChannelsOperationOptions{}
}
...
func (c TeamChannelClient) ListTeamChannels(ctx context.Context, id stable.GroupId, options ListTeamChannelsOperationOptions) (result ListTeamChannelsOperationResponse, err error) {
...
func (c TeamChannelClient) ListTeamChannelsComplete(ctx context.Context, id stable.GroupId, options ListTeamChannelsOperationOptions) (ListTeamChannelsCompleteResult, error) {
	return c.ListTeamChannelsCompleteMatchingPredicate(ctx, id, options, ChannelOperationPredicate{})
}

// ListTeamChannelsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c TeamChannelClient) ListTeamChannelsCompleteMatchingPredicate(ctx context.Context, id stable.GroupId, options ListTeamChannelsOperationOptions, predicate ChannelOperationPredicate) (result ListTeamChannelsCompleteResult, err error) {
	items := make([]stable.Channel, 0)

	resp, err := c.ListTeamChannels(ctx, id, options)
	if err != nil {
