...
func DefaultListThreadPostAttachmentsOperationOptions() ListThreadPostAttachmentsOperationOptions {
	return ListThreadPostAttachmentsOperationOptions{}
}
...
func (c ThreadPostAttachmentClient) ListThreadPostAttachments(ctx context.Context, id stable.GroupIdThreadIdPostId, options ListThreadPostAttachmentsOperationOptions) (result ListThreadPostAttachmentsOperationResponse, err error) {
...
func (c ThreadPostAttachmentClient) ListThreadPostAttachmentsComplete(ctx context.Context, id stable.GroupIdThreadIdPostId, options ListThreadPostAttachmentsOperationOptions) (ListThreadPostAttachmentsCompleteResult, error) {
	return c.ListThreadPostAttachmentsCompleteMatchingPredicate(ctx, id, options, AttachmentOperationPredicate{})
}

// ListThreadPostAttachmentsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c ThreadPostAttachmentClient) ListThreadPostAttachmentsCompleteMatchingPredicate(ctx context.Context, id stable.GroupIdThreadIdPostId, options ListThreadPostAttachmentsOperationOptions, predicate AttachmentOperationPredicate) (result ListThreadPostAttachmentsCompleteResult, err error) {
	items := make([]stable.Attachment, 0)

	resp, err := c.ListThreadPostAttachments(ctx, id, options)
	if err != nil {
