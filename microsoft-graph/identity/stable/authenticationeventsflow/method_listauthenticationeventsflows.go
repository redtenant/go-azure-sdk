...
func DefaultListAuthenticationEventsFlowsOperationOptions() ListAuthenticationEventsFlowsOperationOptions {
	return ListAuthenticationEventsFlowsOperationOptions{}
}
...
// ListAuthenticationEventsFlows - List authenticationEventsFlows. Get a collection of authentication events policies
// that are derived from authenticationEventsFlow. The following derived subtypes are supported: -
...
func (c AuthenticationEventsFlowClient) ListAuthenticationEventsFlows(ctx context.Context, options ListAuthenticationEventsFlowsOperationOptions) (result ListAuthenticationEventsFlowsOperationResponse, err error) {
	opts := client.RequestOptions{
		...
		Pager:         &ListAuthenticationEventsFlowsCustomPager{},
	...
	temp := make([]stable.AuthenticationEventsFlow, 0)
	if values.Values != nil {
		for i, v := range *values.Values {
			val, err := stable.UnmarshalAuthenticationEventsFlowImplementation(v)
			if err != nil {
				err = fmt.Errorf("unmarshalling item %d for stable.AuthenticationEventsFlow (%q): %+v", i, v, err)
				return result, err
			}
...
func (c AuthenticationEventsFlowClient) ListAuthenticationEventsFlowsComplete(ctx context.Context, options ListAuthenticationEventsFlowsOperationOptions) (ListAuthenticationEventsFlowsCompleteResult, error) {
	return c.ListAuthenticationEventsFlowsCompleteMatchingPredicate(ctx, options, AuthenticationEventsFlowOperationPredicate{})
}
...
func (c AuthenticationEventsFlowClient) ListAuthenticationEventsFlowsCompleteMatchingPredicate(ctx context.Context, options ListAuthenticationEventsFlowsOperationOptions, predicate AuthenticationEventsFlowOperationPredicate) (result ListAuthenticationEventsFlowsCompleteResult, err error) {
	items := make([]stable.AuthenticationEventsFlow, 0)

	resp, err := c.ListAuthenticationEventsFlows(ctx, options)
	if err != nil {
	...
	result = ListAuthenticationEventsFlowsCompleteResult{
