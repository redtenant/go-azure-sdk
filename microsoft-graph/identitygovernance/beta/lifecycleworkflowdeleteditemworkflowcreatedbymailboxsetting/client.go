...
func NewLifecycleWorkflowDeletedItemWorkflowCreatedByMailboxSettingClientWithBaseURI(sdkApi sdkEnv.Api) (*LifecycleWorkflowDeletedItemWorkflowCreatedByMailboxSettingClient, error) {
	client, err := msgraph.NewClient(sdkApi, "lifecycleworkflowdeleteditemworkflowcreatedbymailboxsetting", defaultApiVersion)
	if err != nil {
		return nil, fmt.Errorf("instantiating LifecycleWorkflowDeletedItemWorkflowCreatedByMailboxSettingClient: %+v", err)
	}

	return &LifecycleWorkflowDeletedItemWorkflowCreatedByMailboxSettingClient{
		Client: client,
	}, nil
}
