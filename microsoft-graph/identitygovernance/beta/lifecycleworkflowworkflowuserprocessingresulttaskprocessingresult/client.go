...
func NewLifecycleWorkflowWorkflowUserProcessingResultTaskProcessingResultClientWithBaseURI(sdkApi sdkEnv.Api) (*LifecycleWorkflowWorkflowUserProcessingResultTaskProcessingResultClient, error) {
	client, err := msgraph.NewClient(sdkApi, "lifecycleworkflowworkflowuserprocessingresulttaskprocessingresult", defaultApiVersion)
	if err != nil {
		return nil, fmt.Errorf("instantiating LifecycleWorkflowWorkflowUserProcessingResultTaskProcessingResultClient: %+v", err)
	}

	return &LifecycleWorkflowWorkflowUserProcessingResultTaskProcessingResultClient{
		Client: client,
	}, nil
}
