...
// GetRoleManagementAlertConfiguration - Get alertConfiguration from identityGovernance. The configuration of the alert
// in PIM for Microsoft Entra roles. Alert configurations are pre-defined and cannot be created or deleted, but some
// configurations can be modified. Supports $filter for the isEnabled property and $expand.
func (c RoleManagementAlertAlertAlertConfigurationClient) GetRoleManagementAlertConfiguration(ctx context.Context, id beta.IdentityGovernanceRoleManagementAlertAlertId, options GetRoleManagementAlertConfigurationOperationOptions) (result GetRoleManagementAlertConfigurationOperationResponse, err error) {
	opts := client.RequestOptions{
		...
		Path:          fmt.Sprintf("%s/alertConfiguration", id.ID()),
