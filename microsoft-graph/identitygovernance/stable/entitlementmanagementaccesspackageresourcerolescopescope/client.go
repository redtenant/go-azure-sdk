...
func NewEntitlementManagementAccessPackageResourceRoleScopeScopeClientWithBaseURI(sdkApi sdkEnv.Api) (*EntitlementManagementAccessPackageResourceRoleScopeScopeClient, error) {
	client, err := msgraph.NewClient(sdkApi, "entitlementmanagementaccesspackageresourcerolescopescope", defaultApiVersion)
	if err != nil {
		return nil, fmt.Errorf("instantiating EntitlementManagementAccessPackageResourceRoleScopeScopeClient: %+v", err)
	}

	return &EntitlementManagementAccessPackageResourceRoleScopeScopeClient{
		Client: client,
	}, nil
}
