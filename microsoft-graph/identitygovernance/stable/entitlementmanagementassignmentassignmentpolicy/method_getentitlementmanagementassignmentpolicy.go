...
// GetEntitlementManagementAssignmentPolicy - Get assignmentPolicy from identityGovernance. Read-only. Supports $filter
// (eq) on the id property and $expand query parameters.
func (c EntitlementManagementAssignmentAssignmentPolicyClient) GetEntitlementManagementAssignmentPolicy(ctx context.Context, id stable.IdentityGovernanceEntitlementManagementAssignmentId, options GetEntitlementManagementAssignmentPolicyOperationOptions) (result GetEntitlementManagementAssignmentPolicyOperationResponse, err error) {
	opts := client.RequestOptions{
		...
		Path:          fmt.Sprintf("%s/assignmentPolicy", id.ID()),
