...
func NewEntitlementManagementResourceRoleClientWithBaseURI(sdkApi sdkEnv.Api) (*EntitlementManagementResourceRoleClient, error) {
	client, err := msgraph.NewClient(sdkApi, "entitlementmanagementresourcerole", defaultApiVersion)
	if err != nil {
		return nil, fmt.Errorf("instantiating EntitlementManagementResourceRoleClient: %+v", err)
	}

	return &EntitlementManagementResourceRoleClient{
		Client: client,
	}, nil
}
