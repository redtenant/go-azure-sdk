...
func NewEntitlementManagementResourceRoleScopeRoleResourceScopeResourceClientWithBaseURI(sdkApi sdkEnv.Api) (*EntitlementManagementResourceRoleScopeRoleResourceScopeResourceClient, error) {
	client, err := msgraph.NewClient(sdkApi, "entitlementmanagementresourcerolescoperoleresourcescoperesource", defaultApiVersion)
	if err != nil {
		return nil, fmt.Errorf("instantiating EntitlementManagementResourceRoleScopeRoleResourceScopeResourceClient: %+v", err)
	}

	return &EntitlementManagementResourceRoleScopeRoleResourceScopeResourceClient{
		Client: client,
	}, nil
}
