...
func NewEntitlementManagementResourceScopeClientWithBaseURI(sdkApi sdkEnv.Api) (*EntitlementManagementResourceScopeClient, error) {
	client, err := msgraph.NewClient(sdkApi, "entitlementmanagementresourcescope", defaultApiVersion)
	if err != nil {
		return nil, fmt.Errorf("instantiating EntitlementManagementResourceScopeClient: %+v", err)
	}

	return &EntitlementManagementResourceScopeClient{
		Client: client,
	}, nil
}
