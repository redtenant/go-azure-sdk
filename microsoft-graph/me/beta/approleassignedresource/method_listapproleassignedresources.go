...
// ListAppRoleAssignedResources - List appRoleAssignedResources. Get the service principals to which the user has an app
// role assignment either directly or through group membership.
func (c AppRoleAssignedResourceClient) ListAppRoleAssignedResources(ctx context.Context, options ListAppRoleAssignedResourcesOperationOptions) (result ListAppRoleAssignedResourcesOperationResponse, err error) {
...
func (c AppRoleAssignedResourceClient) ListAppRoleAssignedResourcesComplete(ctx context.Context, options ListAppRoleAssignedResourcesOperationOptions) (ListAppRoleAssignedResourcesCompleteResult, error) {
	return c.ListAppRoleAssignedResourcesCompleteMatchingPredicate(ctx, options, ServicePrincipalOperationPredicate{})
}

// ListAppRoleAssignedResourcesCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c AppRoleAssignedResourceClient) ListAppRoleAssignedResourcesCompleteMatchingPredicate(ctx context.Context, options ListAppRoleAssignedResourcesOperationOptions, predicate ServicePrincipalOperationPredicate) (result ListAppRoleAssignedResourcesCompleteResult, err error) {
	items := make([]beta.ServicePrincipal, 0)

	resp, err := c.ListAppRoleAssignedResources(ctx, options)
	if err != nil {
