...
// ListMemberOfs - Get memberOf from me. The groups, directory roles, and administrative units that the user is a member
// of. Read-only. Nullable. Supports $expand.
func (c MemberOfClient) ListMemberOfs(ctx context.Context, options ListMemberOfsOperationOptions) (result ListMemberOfsOperationResponse, err error) {
...
func (c MemberOfClient) ListMemberOfsComplete(ctx context.Context, options ListMemberOfsOperationOptions) (ListMemberOfsCompleteResult, error) {
	return c.ListMemberOfsCompleteMatchingPredicate(ctx, options, DirectoryObjectOperationPredicate{})
}

// ListMemberOfsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c MemberOfClient) ListMemberOfsCompleteMatchingPredicate(ctx context.Context, options ListMemberOfsOperationOptions, predicate DirectoryObjectOperationPredicate) (result ListMemberOfsCompleteResult, err error) {
	items := make([]beta.DirectoryObject, 0)

	resp, err := c.ListMemberOfs(ctx, options)
	if err != nil {
