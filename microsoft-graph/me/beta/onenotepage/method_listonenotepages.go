...
// ListOnenotePages - Get pages from me. The pages in all OneNote notebooks that are owned by the user or group.
// Read-only. Nullable.
func (c OnenotePageClient) ListOnenotePages(ctx context.Context, options ListOnenotePagesOperationOptions) (result ListOnenotePagesOperationResponse, err error) {
...
func (c OnenotePageClient) ListOnenotePagesComplete(ctx context.Context, options ListOnenotePagesOperationOptions) (ListOnenotePagesCompleteResult, error) {
	return c.ListOnenotePagesCompleteMatchingPredicate(ctx, options, OnenotePageOperationPredicate{})
}

// ListOnenotePagesCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c OnenotePageClient) ListOnenotePagesCompleteMatchingPredicate(ctx context.Context, options ListOnenotePagesOperationOptions, predicate OnenotePageOperationPredicate) (result ListOnenotePagesCompleteResult, err error) {
	items := make([]beta.OnenotePage, 0)

	resp, err := c.ListOnenotePages(ctx, options)
	if err != nil {
