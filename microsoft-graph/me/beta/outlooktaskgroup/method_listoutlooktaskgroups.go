...
// ListOutlookTaskGroups - List taskGroups (deprecated). Get all the Outlook task groups in the user's mailbox. The
// response always includes the default task group My Tasks, and any other task groups that have been created in the
// mailbox.
func (c OutlookTaskGroupClient) ListOutlookTaskGroups(ctx context.Context, options ListOutlookTaskGroupsOperationOptions) (result ListOutlookTaskGroupsOperationResponse, err error) {
...
func (c OutlookTaskGroupClient) ListOutlookTaskGroupsComplete(ctx context.Context, options ListOutlookTaskGroupsOperationOptions) (ListOutlookTaskGroupsCompleteResult, error) {
	return c.ListOutlookTaskGroupsCompleteMatchingPredicate(ctx, options, OutlookTaskGroupOperationPredicate{})
}

// ListOutlookTaskGroupsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c OutlookTaskGroupClient) ListOutlookTaskGroupsCompleteMatchingPredicate(ctx context.Context, options ListOutlookTaskGroupsOperationOptions, predicate OutlookTaskGroupOperationPredicate) (result ListOutlookTaskGroupsCompleteResult, err error) {
	items := make([]beta.OutlookTaskGroup, 0)

	resp, err := c.ListOutlookTaskGroups(ctx, options)
	if err != nil {
