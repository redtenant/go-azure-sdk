...
// ListOwnedObjects - List ownedObjects. Get the list of directory objects that are owned by the user.
func (c OwnedObjectClient) ListOwnedObjects(ctx context.Context, options ListOwnedObjectsOperationOptions) (result ListOwnedObjectsOperationResponse, err error) {
...
func (c OwnedObjectClient) ListOwnedObjectsComplete(ctx context.Context, options ListOwnedObjectsOperationOptions) (ListOwnedObjectsCompleteResult, error) {
	return c.ListOwnedObjectsCompleteMatchingPredicate(ctx, options, DirectoryObjectOperationPredicate{})
}

// ListOwnedObjectsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c OwnedObjectClient) ListOwnedObjectsCompleteMatchingPredicate(ctx context.Context, options ListOwnedObjectsOperationOptions, predicate DirectoryObjectOperationPredicate) (result ListOwnedObjectsCompleteResult, err error) {
	items := make([]beta.DirectoryObject, 0)

	resp, err := c.ListOwnedObjects(ctx, options)
	if err != nil {
