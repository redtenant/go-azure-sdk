...
// associated with. Currently, a user can be associated with a team in two different ways: * A user can be a direct
// member of a team. * A user can be a member of a shared channel that is hosted inside a team.
...
func (c TeamworkAssociatedTeamClient) ListTeamworkAssociatedTeamsComplete(ctx context.Context, options ListTeamworkAssociatedTeamsOperationOptions) (ListTeamworkAssociatedTeamsCompleteResult, error) {
	return c.ListTeamworkAssociatedTeamsCompleteMatchingPredicate(ctx, options, AssociatedTeamInfoOperationPredicate{})
}

// ListTeamworkAssociatedTeamsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c TeamworkAssociatedTeamClient) ListTeamworkAssociatedTeamsCompleteMatchingPredicate(ctx context.Context, options ListTeamworkAssociatedTeamsOperationOptions, predicate AssociatedTeamInfoOperationPredicate) (result ListTeamworkAssociatedTeamsCompleteResult, err error) {
	items := make([]beta.AssociatedTeamInfo, 0)

	resp, err := c.ListTeamworkAssociatedTeams(ctx, options)
	if err != nil {
