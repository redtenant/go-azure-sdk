...
func DefaultListTransitiveReportsOperationOptions() ListTransitiveReportsOperationOptions {
	return ListTransitiveReportsOperationOptions{}
}
...
// ListTransitiveReports - Get transitiveReports from me. The transitive reports for a user. Read-only.
func (c TransitiveReportClient) ListTransitiveReports(ctx context.Context, options ListTransitiveReportsOperationOptions) (result ListTransitiveReportsOperationResponse, err error) {
...
func (c TransitiveReportClient) ListTransitiveReportsComplete(ctx context.Context, options ListTransitiveReportsOperationOptions) (ListTransitiveReportsCompleteResult, error) {
	return c.ListTransitiveReportsCompleteMatchingPredicate(ctx, options, DirectoryObjectOperationPredicate{})
}

// ListTransitiveReportsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c TransitiveReportClient) ListTransitiveReportsCompleteMatchingPredicate(ctx context.Context, options ListTransitiveReportsOperationOptions, predicate DirectoryObjectOperationPredicate) (result ListTransitiveReportsCompleteResult, err error) {
	items := make([]beta.DirectoryObject, 0)

	resp, err := c.ListTransitiveReports(ctx, options)
	if err != nil {
