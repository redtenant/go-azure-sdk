...
// ListAuthenticationOperations - Get operations from me. Represents the status of a long-running operation, such as a
// password reset operation.
func (c AuthenticationOperationClient) ListAuthenticationOperations(ctx context.Context, options ListAuthenticationOperationsOperationOptions) (result ListAuthenticationOperationsOperationResponse, err error) {
...
func (c AuthenticationOperationClient) ListAuthenticationOperationsComplete(ctx context.Context, options ListAuthenticationOperationsOperationOptions) (ListAuthenticationOperationsCompleteResult, error) {
	return c.ListAuthenticationOperationsCompleteMatchingPredicate(ctx, options, LongRunningOperationOperationPredicate{})
}

// ListAuthenticationOperationsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c AuthenticationOperationClient) ListAuthenticationOperationsCompleteMatchingPredicate(ctx context.Context, options ListAuthenticationOperationsOperationOptions, predicate LongRunningOperationOperationPredicate) (result ListAuthenticationOperationsCompleteResult, err error) {
	items := make([]stable.LongRunningOperation, 0)

	resp, err := c.ListAuthenticationOperations(ctx, options)
	if err != nil {
