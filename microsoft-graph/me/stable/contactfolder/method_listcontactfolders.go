...
// ListContactFolders - List contactFolders. Get the contact folder collection in the default Contacts folder of the
// signed-in user.
func (c ContactFolderClient) ListContactFolders(ctx context.Context, options ListContactFoldersOperationOptions) (result ListContactFoldersOperationResponse, err error) {
...
func (c ContactFolderClient) ListContactFoldersComplete(ctx context.Context, options ListContactFoldersOperationOptions) (ListContactFoldersCompleteResult, error) {
	return c.ListContactFoldersCompleteMatchingPredicate(ctx, options, ContactFolderOperationPredicate{})
}

// ListContactFoldersCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c ContactFolderClient) ListContactFoldersCompleteMatchingPredicate(ctx context.Context, options ListContactFoldersOperationOptions, predicate ContactFolderOperationPredicate) (result ListContactFoldersCompleteResult, err error) {
	items := make([]stable.ContactFolder, 0)

	resp, err := c.ListContactFolders(ctx, options)
	if err != nil {
