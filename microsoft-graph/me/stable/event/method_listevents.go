...
// ListEvents - List events. Get a list of event objects in the user's mailbox. The list contains single instance
// meetings and series masters. To get expanded event instances, you can get the calendar view, or get the instances of
// an event. Currently, this operation returns event bodies in only HTML format. There are two scenarios where an app
// can get events in another user's calendar
...
func (c EventClient) ListEventsComplete(ctx context.Context, options ListEventsOperationOptions) (ListEventsCompleteResult, error) {
	return c.ListEventsCompleteMatchingPredicate(ctx, options, EventOperationPredicate{})
}
...
func (c EventClient) ListEventsCompleteMatchingPredicate(ctx context.Context, options ListEventsOperationOptions, predicate EventOperationPredicate) (result ListEventsCompleteResult, err error) {
	...
	resp, err := c.ListEvents(ctx, options)
	if err != nil {
