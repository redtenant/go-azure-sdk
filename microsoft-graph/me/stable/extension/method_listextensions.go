...
// ListExtensions - Get extensions from me. The collection of open extensions defined for the user. Read-only. Supports
// $expand. Nullable.
func (c ExtensionClient) ListExtensions(ctx context.Context, options ListExtensionsOperationOptions) (result ListExtensionsOperationResponse, err error) {
...
func (c ExtensionClient) ListExtensionsComplete(ctx context.Context, options ListExtensionsOperationOptions) (ListExtensionsCompleteResult, error) {
	return c.ListExtensionsCompleteMatchingPredicate(ctx, options, ExtensionOperationPredicate{})
}

// ListExtensionsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c ExtensionClient) ListExtensionsCompleteMatchingPredicate(ctx context.Context, options ListExtensionsOperationOptions, predicate ExtensionOperationPredicate) (result ListExtensionsCompleteResult, err error) {
	items := make([]stable.Extension, 0)

	resp, err := c.ListExtensions(ctx, options)
	if err != nil {
