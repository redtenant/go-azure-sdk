...
// to the user.
func (c ManagedAppRegistrationClient) GetManagedAppRegistration(ctx context.Context, id stable.MeManagedAppRegistrationId, options GetManagedAppRegistrationOperationOptions) (result GetManagedAppRegistrationOperationResponse, err error) {
	...
	model, err := stable.UnmarshalManagedAppRegistrationImplementation(respObj)
	if err != nil {
		return
	}
	result.Model = model
