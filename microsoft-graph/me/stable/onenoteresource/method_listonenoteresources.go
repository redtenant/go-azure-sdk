...
// ListOnenoteResources - Get resources from me. The image and other file resources in OneNote pages. Getting a
// resources collection isn't supported, but you can get the binary content of a specific resource. Read-only. Nullable.
func (c OnenoteResourceClient) ListOnenoteResources(ctx context.Context, options ListOnenoteResourcesOperationOptions) (result ListOnenoteResourcesOperationResponse, err error) {
...
func (c OnenoteResourceClient) ListOnenoteResourcesComplete(ctx context.Context, options ListOnenoteResourcesOperationOptions) (ListOnenoteResourcesCompleteResult, error) {
	return c.ListOnenoteResourcesCompleteMatchingPredicate(ctx, options, OnenoteResourceOperationPredicate{})
}

// ListOnenoteResourcesCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c OnenoteResourceClient) ListOnenoteResourcesCompleteMatchingPredicate(ctx context.Context, options ListOnenoteResourcesOperationOptions, predicate OnenoteResourceOperationPredicate) (result ListOnenoteResourcesCompleteResult, err error) {
	items := make([]stable.OnenoteResource, 0)

	resp, err := c.ListOnenoteResources(ctx, options)
	if err != nil {
