...
// GetOwnedObject - Get ownedObjects from me. Directory objects the user owns. Read-only. Nullable. Supports $expand,
// $select nested in $expand, and $filter (/$count eq 0, /$count ne 0, /$count eq 1, /$count ne 1).
func (c OwnedObjectClient) GetOwnedObject(ctx context.Context, id stable.MeOwnedObjectId, options GetOwnedObjectOperationOptions) (result GetOwnedObjectOperationResponse, err error) {
