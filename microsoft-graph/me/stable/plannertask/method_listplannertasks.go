...
// ListPlannerTasks - List tasks. Retrieve a list of plannertask objects assigned to a User.
func (c PlannerTaskClient) ListPlannerTasks(ctx context.Context, options ListPlannerTasksOperationOptions) (result ListPlannerTasksOperationResponse, err error) {
...
func (c PlannerTaskClient) ListPlannerTasksComplete(ctx context.Context, options ListPlannerTasksOperationOptions) (ListPlannerTasksCompleteResult, error) {
	return c.ListPlannerTasksCompleteMatchingPredicate(ctx, options, PlannerTaskOperationPredicate{})
}

// ListPlannerTasksCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c PlannerTaskClient) ListPlannerTasksCompleteMatchingPredicate(ctx context.Context, options ListPlannerTasksOperationOptions, predicate PlannerTaskOperationPredicate) (result ListPlannerTasksCompleteResult, err error) {
	items := make([]stable.PlannerTask, 0)

	resp, err := c.ListPlannerTasks(ctx, options)
	if err != nil {
