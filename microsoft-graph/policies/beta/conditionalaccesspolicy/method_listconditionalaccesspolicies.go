...
// ListConditionalAccessPolicies - Get conditionalAccessPolicies from policies. The custom rules that define an access
// scenario.
func (c ConditionalAccessPolicyClient) ListConditionalAccessPolicies(ctx context.Context, options ListConditionalAccessPoliciesOperationOptions) (result ListConditionalAccessPoliciesOperationResponse, err error) {
	...
	temp := make([]beta.ConditionalAccessPolicy, 0)
	if values.Values != nil {
		for i, v := range *values.Values {
			val, err := beta.UnmarshalConditionalAccessPolicyImplementation(v)
			if err != nil {
				err = fmt.Errorf("unmarshalling item %d for beta.ConditionalAccessPolicy (%q): %+v", i, v, err)
				return result, err
			}
