...
func NewEnterpriseAppRoleAssignmentScheduleInstanceClientWithBaseURI(sdkApi sdkEnv.Api) (*EnterpriseAppRoleAssignmentScheduleInstanceClient, error) {
	client, err := msgraph.NewClient(sdkApi, "enterpriseapproleassignmentscheduleinstance", defaultApiVersion)
	if err != nil {
		return nil, fmt.Errorf("instantiating EnterpriseAppRoleAssignmentScheduleInstanceClient: %+v", err)
	}

	return &EnterpriseAppRoleAssignmentScheduleInstanceClient{
		Client: client,
	}, nil
}
