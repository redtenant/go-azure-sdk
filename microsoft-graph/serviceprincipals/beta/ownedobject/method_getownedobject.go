...
// GetOwnedObject - Get ownedObjects from servicePrincipals. Directory objects that are owned by this service principal.
// Read-only. Nullable. Supports $expand and $filter (/$count eq 0, /$count ne 0, /$count eq 1, /$count ne 1).
func (c OwnedObjectClient) GetOwnedObject(ctx context.Context, id beta.ServicePrincipalIdOwnedObjectId, options GetOwnedObjectOperationOptions) (result GetOwnedObjectOperationResponse, err error) {
