...
// ListCreatedObjects - Get createdObjects from servicePrincipals. Directory objects created by this service principal.
// Read-only. Nullable.
func (c CreatedObjectClient) ListCreatedObjects(ctx context.Context, id stable.ServicePrincipalId, options ListCreatedObjectsOperationOptions) (result ListCreatedObjectsOperationResponse, err error) {
...
func (c CreatedObjectClient) ListCreatedObjectsComplete(ctx context.Context, id stable.ServicePrincipalId, options ListCreatedObjectsOperationOptions) (ListCreatedObjectsCompleteResult, error) {
	return c.ListCreatedObjectsCompleteMatchingPredicate(ctx, id, options, DirectoryObjectOperationPredicate{})
}

// ListCreatedObjectsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c CreatedObjectClient) ListCreatedObjectsCompleteMatchingPredicate(ctx context.Context, id stable.ServicePrincipalId, options ListCreatedObjectsOperationOptions, predicate DirectoryObjectOperationPredicate) (result ListCreatedObjectsCompleteResult, err error) {
	items := make([]stable.DirectoryObject, 0)

	resp, err := c.ListCreatedObjects(ctx, id, options)
	if err != nil {
