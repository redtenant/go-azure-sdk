...
// GetFederatedIdentityCredential - Get federatedIdentityCredentials from servicePrincipals. Federated identities for a
// specific type of service principal - managed identity. Supports $expand and $filter (/$count eq 0, /$count ne 0).
func (c FederatedIdentityCredentialClient) GetFederatedIdentityCredential(ctx context.Context, id stable.ServicePrincipalIdFederatedIdentityCredentialId, options GetFederatedIdentityCredentialOperationOptions) (result GetFederatedIdentityCredentialOperationResponse, err error) {
