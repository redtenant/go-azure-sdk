...
// ListOwnedObjects - Get ownedObjects from servicePrincipals. Directory objects that this service principal owns.
// Read-only. Nullable. Supports $expand, $select nested in $expand, and $filter (/$count eq 0, /$count ne 0, /$count eq
// 1, /$count ne 1).
func (c OwnedObjectClient) ListOwnedObjects(ctx context.Context, id stable.ServicePrincipalId, options ListOwnedObjectsOperationOptions) (result ListOwnedObjectsOperationResponse, err error) {
...
func (c OwnedObjectClient) ListOwnedObjectsComplete(ctx context.Context, id stable.ServicePrincipalId, options ListOwnedObjectsOperationOptions) (ListOwnedObjectsCompleteResult, error) {
