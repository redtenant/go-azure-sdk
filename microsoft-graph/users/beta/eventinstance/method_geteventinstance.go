...
// GetEventInstance - Get instances from users. The occurrences of a recurring series, if the event is a series master.
// This property includes occurrences that are part of the recurrence pattern, and exceptions that have been modified,
// but doesn't include occurrences that have been canceled from the series. Navigation property. Read-only. Nullable.
func (c EventInstanceClient) GetEventInstance(ctx context.Context, id beta.UserIdEventIdInstanceId, options GetEventInstanceOperationOptions) (result GetEventInstanceOperationResponse, err error) {
