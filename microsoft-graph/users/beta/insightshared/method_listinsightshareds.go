...
// ListInsightShareds - Get shared from users. Access this property from the derived type itemInsights.
func (c InsightSharedClient) ListInsightShareds(ctx context.Context, id beta.UserId, options ListInsightSharedsOperationOptions) (result ListInsightSharedsOperationResponse, err error) {
...
func (c InsightSharedClient) ListInsightSharedsComplete(ctx context.Context, id beta.UserId, options ListInsightSharedsOperationOptions) (ListInsightSharedsCompleteResult, error) {
	return c.ListInsightSharedsCompleteMatchingPredicate(ctx, id, options, SharedInsightOperationPredicate{})
}

// ListInsightSharedsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c InsightSharedClient) ListInsightSharedsCompleteMatchingPredicate(ctx context.Context, id beta.UserId, options ListInsightSharedsOperationOptions, predicate SharedInsightOperationPredicate) (result ListInsightSharedsCompleteResult, err error) {
	items := make([]beta.SharedInsight, 0)

	resp, err := c.ListInsightShareds(ctx, id, options)
	if err != nil {
