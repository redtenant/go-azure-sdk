...
// GetMailboxSetting - Get mailboxSettings property value. Settings for the primary mailbox of the signed-in user. You
// can get or update settings for sending automatic replies to incoming messages, locale, and time zone. For more
// information, see User preferences for languages and regional formats. Returned only on $select.
func (c MailboxSettingClient) GetMailboxSetting(ctx context.Context, id beta.UserId, options GetMailboxSettingOperationOptions) (result GetMailboxSettingOperationResponse, err error) {
