...
func DefaultListOutlookTaskAttachmentsOperationOptions() ListOutlookTaskAttachmentsOperationOptions {
	return ListOutlookTaskAttachmentsOperationOptions{}
}
...
// ListOutlookTaskAttachments - List attachments (deprecated). Get a list of attachment objects attached to an Outlook
// task.
func (c OutlookTaskAttachmentClient) ListOutlookTaskAttachments(ctx context.Context, id beta.UserIdOutlookTaskId, options ListOutlookTaskAttachmentsOperationOptions) (result ListOutlookTaskAttachmentsOperationResponse, err error) {
...
func (c OutlookTaskAttachmentClient) ListOutlookTaskAttachmentsComplete(ctx context.Context, id beta.UserIdOutlookTaskId, options ListOutlookTaskAttachmentsOperationOptions) (ListOutlookTaskAttachmentsCompleteResult, error) {
	return c.ListOutlookTaskAttachmentsCompleteMatchingPredicate(ctx, id, options, AttachmentOperationPredicate{})
}

// ListOutlookTaskAttachmentsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c OutlookTaskAttachmentClient) ListOutlookTaskAttachmentsCompleteMatchingPredicate(ctx context.Context, id beta.UserIdOutlookTaskId, options ListOutlookTaskAttachmentsOperationOptions, predicate AttachmentOperationPredicate) (result ListOutlookTaskAttachmentsCompleteResult, err error) {
	items := make([]beta.Attachment, 0)

	resp, err := c.ListOutlookTaskAttachments(ctx, id, options)
	if err != nil {
