...
// ListProfileAccounts - Get account from users
func (c ProfileAccountClient) ListProfileAccounts(ctx context.Context, id beta.UserId, options ListProfileAccountsOperationOptions) (result ListProfileAccountsOperationResponse, err error) {
...
func (c ProfileAccountClient) ListProfileAccountsComplete(ctx context.Context, id beta.UserId, options ListProfileAccountsOperationOptions) (ListProfileAccountsCompleteResult, error) {
	return c.ListProfileAccountsCompleteMatchingPredicate(ctx, id, options, UserAccountInformationOperationPredicate{})
}

// ListProfileAccountsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c ProfileAccountClient) ListProfileAccountsCompleteMatchingPredicate(ctx context.Context, id beta.UserId, options ListProfileAccountsOperationOptions, predicate UserAccountInformationOperationPredicate) (result ListProfileAccountsCompleteResult, err error) {
	items := make([]beta.UserAccountInformation, 0)

	resp, err := c.ListProfileAccounts(ctx, id, options)
	if err != nil {
