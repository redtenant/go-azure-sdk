...
// ListProfileNames - Get names from users. Represents the names a user has added to their profile.
func (c ProfileNameClient) ListProfileNames(ctx context.Context, id beta.UserId, options ListProfileNamesOperationOptions) (result ListProfileNamesOperationResponse, err error) {
...
func (c ProfileNameClient) ListProfileNamesComplete(ctx context.Context, id beta.UserId, options ListProfileNamesOperationOptions) (ListProfileNamesCompleteResult, error) {
	return c.ListProfileNamesCompleteMatchingPredicate(ctx, id, options, PersonNameOperationPredicate{})
}

// ListProfileNamesCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c ProfileNameClient) ListProfileNamesCompleteMatchingPredicate(ctx context.Context, id beta.UserId, options ListProfileNamesOperationOptions, predicate PersonNameOperationPredicate) (result ListProfileNamesCompleteResult, err error) {
	items := make([]beta.PersonName, 0)

	resp, err := c.ListProfileNames(ctx, id, options)
	if err != nil {
