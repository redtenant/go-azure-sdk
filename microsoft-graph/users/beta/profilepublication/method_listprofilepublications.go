...
func DefaultListProfilePublicationsOperationOptions() ListProfilePublicationsOperationOptions {
	return ListProfilePublicationsOperationOptions{}
}
...
// ListProfilePublications - Get publications from users. Represents details of any publications a user has added to
// their profile.
func (c ProfilePublicationClient) ListProfilePublications(ctx context.Context, id beta.UserId, options ListProfilePublicationsOperationOptions) (result ListProfilePublicationsOperationResponse, err error) {
...
func (c ProfilePublicationClient) ListProfilePublicationsComplete(ctx context.Context, id beta.UserId, options ListProfilePublicationsOperationOptions) (ListProfilePublicationsCompleteResult, error) {
	return c.ListProfilePublicationsCompleteMatchingPredicate(ctx, id, options, ItemPublicationOperationPredicate{})
}

// ListProfilePublicationsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c ProfilePublicationClient) ListProfilePublicationsCompleteMatchingPredicate(ctx context.Context, id beta.UserId, options ListProfilePublicationsOperationOptions, predicate ItemPublicationOperationPredicate) (result ListProfilePublicationsCompleteResult, err error) {
	items := make([]beta.ItemPublication, 0)

	resp, err := c.ListProfilePublications(ctx, id, options)
	if err != nil {
