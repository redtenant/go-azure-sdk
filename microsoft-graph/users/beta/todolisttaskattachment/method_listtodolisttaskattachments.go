...
func (c TodoListTaskAttachmentClient) ListTodoListTaskAttachments(ctx context.Context, id beta.UserIdTodoListIdTaskId, options ListTodoListTaskAttachmentsOperationOptions) (result ListTodoListTaskAttachmentsOperationResponse, err error) {
	...
	temp := make([]beta.AttachmentBase, 0)
	if values.Values != nil {
		for i, v := range *values.Values {
			val, err := beta.UnmarshalAttachmentBaseImplementation(v)
			if err != nil {
				err = fmt.Errorf("unmarshalling item %d for beta.AttachmentBase (%q): %+v", i, v, err)
				return result, err
			}
...
func (c TodoListTaskAttachmentClient) ListTodoListTaskAttachmentsComplete(ctx context.Context, id beta.UserIdTodoListIdTaskId, options ListTodoListTaskAttachmentsOperationOptions) (ListTodoListTaskAttachmentsCompleteResult, error) {
...
func (c TodoListTaskAttachmentClient) ListTodoListTaskAttachmentsCompleteMatchingPredicate(ctx context.Context, id beta.UserIdTodoListIdTaskId, options ListTodoListTaskAttachmentsOperationOptions, predicate AttachmentBaseOperationPredicate) (result ListTodoListTaskAttachmentsCompleteResult, err error) {
