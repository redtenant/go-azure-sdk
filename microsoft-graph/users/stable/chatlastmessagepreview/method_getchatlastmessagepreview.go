...
// GetChatLastMessagePreview - Get lastMessagePreview from users. Preview of the last message sent in the chat. Null if
// no messages were sent in the chat. Currently, only the list chats operation supports this property.
func (c ChatLastMessagePreviewClient) GetChatLastMessagePreview(ctx context.Context, id stable.UserIdChatId, options GetChatLastMessagePreviewOperationOptions) (result GetChatLastMessagePreviewOperationResponse, err error) {
	...
	var model stable.ChatMessageInfo
	result.Model = &model
