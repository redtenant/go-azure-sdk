...
func (c MailFolderClient) ListMailFolders(ctx context.Context, id stable.UserId, options ListMailFoldersOperationOptions) (result ListMailFoldersOperationResponse, err error) {
...
func (c MailFolderClient) ListMailFoldersComplete(ctx context.Context, id stable.UserId, options ListMailFoldersOperationOptions) (ListMailFoldersCompleteResult, error) {
	return c.ListMailFoldersCompleteMatchingPredicate(ctx, id, options, MailFolderOperationPredicate{})
}

// ListMailFoldersCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c MailFolderClient) ListMailFoldersCompleteMatchingPredicate(ctx context.Context, id stable.UserId, options ListMailFoldersOperationOptions, predicate MailFolderOperationPredicate) (result ListMailFoldersCompleteResult, err error) {
	items := make([]stable.MailFolder, 0)

	resp, err := c.ListMailFolders(ctx, id, options)
	if err != nil {
