...
func DefaultReplyMailFolderMessageOperationOptions() ReplyMailFolderMessageOperationOptions {
	return ReplyMailFolderMessageOperationOptions{}
}
...
// ReplyMailFolderMessage - Invoke action reply. Reply to the sender of a message using either JSON or MIME format. When
...
// saves the message in the Sent Items folder. Alternatively, create a draft to reply to an existing message and send it
// later.
func (c MailFolderMessageClient) ReplyMailFolderMessage(ctx context.Context, id stable.UserIdMailFolderIdMessageId, input ReplyMailFolderMessageRequest, options ReplyMailFolderMessageOperationOptions) (result ReplyMailFolderMessageOperationResponse, err error) {
