...
func DefaultListMailFolderMessageRulesOperationOptions() ListMailFolderMessageRulesOperationOptions {
	return ListMailFolderMessageRulesOperationOptions{}
}
...
// ListMailFolderMessageRules - Get messageRules from users. The collection of rules that apply to the user's Inbox
// folder.
func (c MailFolderMessageRuleClient) ListMailFolderMessageRules(ctx context.Context, id stable.UserIdMailFolderId, options ListMailFolderMessageRulesOperationOptions) (result ListMailFolderMessageRulesOperationResponse, err error) {
...
func (c MailFolderMessageRuleClient) ListMailFolderMessageRulesComplete(ctx context.Context, id stable.UserIdMailFolderId, options ListMailFolderMessageRulesOperationOptions) (ListMailFolderMessageRulesCompleteResult, error) {
	return c.ListMailFolderMessageRulesCompleteMatchingPredicate(ctx, id, options, MessageRuleOperationPredicate{})
}

// ListMailFolderMessageRulesCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c MailFolderMessageRuleClient) ListMailFolderMessageRulesCompleteMatchingPredicate(ctx context.Context, id stable.UserIdMailFolderId, options ListMailFolderMessageRulesOperationOptions, predicate MessageRuleOperationPredicate) (result ListMailFolderMessageRulesCompleteResult, err error) {
	items := make([]stable.MessageRule, 0)

	resp, err := c.ListMailFolderMessageRules(ctx, id, options)
	if err != nil {
