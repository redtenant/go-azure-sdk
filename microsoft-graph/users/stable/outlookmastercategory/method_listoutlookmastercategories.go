...
// ListOutlookMasterCategories - Get masterCategories from users. A list of categories defined for the user.
func (c OutlookMasterCategoryClient) ListOutlookMasterCategories(ctx context.Context, id stable.UserId, options ListOutlookMasterCategoriesOperationOptions) (result ListOutlookMasterCategoriesOperationResponse, err error) {
...
func (c OutlookMasterCategoryClient) ListOutlookMasterCategoriesComplete(ctx context.Context, id stable.UserId, options ListOutlookMasterCategoriesOperationOptions) (ListOutlookMasterCategoriesCompleteResult, error) {
	return c.ListOutlookMasterCategoriesCompleteMatchingPredicate(ctx, id, options, OutlookCategoryOperationPredicate{})
}

// ListOutlookMasterCategoriesCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c OutlookMasterCategoryClient) ListOutlookMasterCategoriesCompleteMatchingPredicate(ctx context.Context, id stable.UserId, options ListOutlookMasterCategoriesOperationOptions, predicate OutlookCategoryOperationPredicate) (result ListOutlookMasterCategoriesCompleteResult, err error) {
	items := make([]stable.OutlookCategory, 0)

	resp, err := c.ListOutlookMasterCategories(ctx, id, options)
	if err != nil {
