...
// GetOwnedDevice - Get ownedDevices from users. Devices the user owns. Read-only. Nullable. Supports $expand and
// $filter (/$count eq 0, /$count ne 0, /$count eq 1, /$count ne 1).
func (c OwnedDeviceClient) GetOwnedDevice(ctx context.Context, id stable.UserIdOwnedDeviceId, options GetOwnedDeviceOperationOptions) (result GetOwnedDeviceOperationResponse, err error) {
