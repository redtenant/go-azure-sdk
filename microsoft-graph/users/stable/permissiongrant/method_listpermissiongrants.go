...
// ListPermissionGrants - List permissionGrants of a user. List all resource-specific permission grants of a user. This
// list specifies the Microsoft Entra apps that have access to the user, along with the corresponding resource-specific
// access that each app has.
func (c PermissionGrantClient) ListPermissionGrants(ctx context.Context, id stable.UserId, options ListPermissionGrantsOperationOptions) (result ListPermissionGrantsOperationResponse, err error) {
...
func (c PermissionGrantClient) ListPermissionGrantsComplete(ctx context.Context, id stable.UserId, options ListPermissionGrantsOperationOptions) (ListPermissionGrantsCompleteResult, error) {
	return c.ListPermissionGrantsCompleteMatchingPredicate(ctx, id, options, ResourceSpecificPermissionGrantOperationPredicate{})
}

// ListPermissionGrantsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c PermissionGrantClient) ListPermissionGrantsCompleteMatchingPredicate(ctx context.Context, id stable.UserId, options ListPermissionGrantsOperationOptions, predicate ResourceSpecificPermissionGrantOperationPredicate) (result ListPermissionGrantsCompleteResult, err error) {
	items := make([]stable.ResourceSpecificPermissionGrant, 0)

	resp, err := c.ListPermissionGrants(ctx, id, options)
	if err != nil {
