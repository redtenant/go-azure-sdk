...
// ListPlannerPlans - Get plans from users. Read-only. Nullable. Returns the plannerTasks assigned to the user.
func (c PlannerPlanClient) ListPlannerPlans(ctx context.Context, id stable.UserId, options ListPlannerPlansOperationOptions) (result ListPlannerPlansOperationResponse, err error) {
...
func (c PlannerPlanClient) ListPlannerPlansComplete(ctx context.Context, id stable.UserId, options ListPlannerPlansOperationOptions) (ListPlannerPlansCompleteResult, error) {
	return c.ListPlannerPlansCompleteMatchingPredicate(ctx, id, options, PlannerPlanOperationPredicate{})
}

// ListPlannerPlansCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c PlannerPlanClient) ListPlannerPlansCompleteMatchingPredicate(ctx context.Context, id stable.UserId, options ListPlannerPlansOperationOptions, predicate PlannerPlanOperationPredicate) (result ListPlannerPlansCompleteResult, err error) {
	items := make([]stable.PlannerPlan, 0)

	resp, err := c.ListPlannerPlans(ctx, id, options)
	if err != nil {
