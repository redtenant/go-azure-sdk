...
func (c InvoicesClient) DownloadMultipleBillingProfileInvoicesThenPoll(ctx context.Context, id BillingProfileId, input []string) error {
	result, err := c.DownloadMultipleBillingProfileInvoices(ctx, id, input)
	if err != nil {
		return fmt.Errorf("performing DownloadMultipleBillingProfileInvoices: %+v", err)
	}

	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after DownloadMultipleBillingProfileInvoices: %+v", err)
	}
