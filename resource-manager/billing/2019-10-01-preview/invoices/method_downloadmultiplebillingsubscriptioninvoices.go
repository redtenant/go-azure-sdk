...
func (c InvoicesClient) DownloadMultipleBillingSubscriptionInvoicesThenPoll(ctx context.Context, id BillingSubscriptionId, input []string) error {
	result, err := c.DownloadMultipleBillingSubscriptionInvoices(ctx, id, input)
	if err != nil {
		return fmt.Errorf("performing DownloadMultipleBillingSubscriptionInvoices: %+v", err)
	}

	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after DownloadMultipleBillingSubscriptionInvoices: %+v", err)
	}
