...
func (c PriceSheetClient) DownloadByBillingProfileThenPoll(ctx context.Context, id BillingProfileId) error {
	result, err := c.DownloadByBillingProfile(ctx, id)
	if err != nil {
		return fmt.Errorf("performing DownloadByBillingProfile: %+v", err)
	}

	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after DownloadByBillingProfile: %+v", err)
	}
