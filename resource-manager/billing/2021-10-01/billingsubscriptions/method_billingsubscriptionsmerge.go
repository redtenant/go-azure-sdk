...
type BillingSubscriptionsMergeOperationResponse struct {
...
func (c BillingSubscriptionsClient) BillingSubscriptionsMergeThenPoll(ctx context.Context, id BillingSubscriptionId, input BillingSubscriptionMergeRequest) error {
	result, err := c.BillingSubscriptionsMerge(ctx, id, input)
	if err != nil {
		return fmt.Errorf("performing BillingSubscriptionsMerge: %+v", err)
	}

	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after BillingSubscriptionsMerge: %+v", err)
	}
