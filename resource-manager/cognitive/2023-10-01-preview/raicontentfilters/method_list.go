...
type ListOperationResponse struct {
	...
	OData        *odata.OData
	Model        *[]RaiContentFilter
}

type ListCompleteResult struct {
	LatestHttpResponse *http.Response
	Items              []RaiContentFilter
}
...
func (c RaiContentFiltersClient) List(ctx context.Context, id LocationId) (result ListOperationResponse, err error) {
	opts := client.RequestOptions{
		...
		Path:       fmt.Sprintf("%s/raiContentFilters", id.ID()),
	}
	...
	var values struct {
		Values *[]RaiContentFilter `json:"value"`
	}
...
func (c RaiContentFiltersClient) ListComplete(ctx context.Context, id LocationId) (ListCompleteResult, error) {
	return c.ListCompleteMatchingPredicate(ctx, id, RaiContentFilterOperationPredicate{})
}
