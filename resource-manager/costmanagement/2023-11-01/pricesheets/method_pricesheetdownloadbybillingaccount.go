...
func (c PriceSheetsClient) PriceSheetDownloadByBillingAccountThenPoll(ctx context.Context, id BillingPeriodId) error {
	result, err := c.PriceSheetDownloadByBillingAccount(ctx, id)
	if err != nil {
		return fmt.Errorf("performing PriceSheetDownloadByBillingAccount: %+v", err)
	}

	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after PriceSheetDownloadByBillingAccount: %+v", err)
	}
