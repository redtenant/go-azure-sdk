...
func (c PriceSheetsClient) PriceSheetDownloadByInvoiceThenPoll(ctx context.Context, id InvoiceId) error {
	result, err := c.PriceSheetDownloadByInvoice(ctx, id)
	if err != nil {
		return fmt.Errorf("performing PriceSheetDownloadByInvoice: %+v", err)
	}

	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after PriceSheetDownloadByInvoice: %+v", err)
	}
