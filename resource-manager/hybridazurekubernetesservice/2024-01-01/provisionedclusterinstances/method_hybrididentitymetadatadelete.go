...
func (c ProvisionedClusterInstancesClient) HybridIdentityMetadataDeleteThenPoll(ctx context.Context, id commonids.ScopeId) error {
	result, err := c.HybridIdentityMetadataDelete(ctx, id)
	if err != nil {
		return fmt.Errorf("performing HybridIdentityMetadataDelete: %+v", err)
	}

	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after HybridIdentityMetadataDelete: %+v", err)
	}
