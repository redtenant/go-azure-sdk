...
type VolumesPerformReplicationTransferOperationResponse struct {
...
func (c VolumesOnPremMigrationClient) VolumesPerformReplicationTransferThenPoll(ctx context.Context, id VolumeId) error {
	result, err := c.VolumesPerformReplicationTransfer(ctx, id)
	if err != nil {
		return fmt.Errorf("performing VolumesPerformReplicationTransfer: %+v", err)
	}

	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after VolumesPerformReplicationTransfer: %+v", err)
	}
