...
func (c VirtualWANsClient) NetworkVirtualApplianceConnectionsDeleteThenPoll(ctx context.Context, id NetworkVirtualApplianceConnectionId) error {
	result, err := c.NetworkVirtualApplianceConnectionsDelete(ctx, id)
	if err != nil {
		return fmt.Errorf("performing NetworkVirtualApplianceConnectionsDelete: %+v", err)
	}

	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after NetworkVirtualApplianceConnectionsDelete: %+v", err)
	}
