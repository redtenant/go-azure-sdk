...
type DeleteDataOperationResponse struct {
...
func (c DataTypesClient) DeleteDataThenPoll(ctx context.Context, id DataTypeId, input interface{}) error {
	result, err := c.DeleteData(ctx, id, input)
	if err != nil {
		return fmt.Errorf("performing DeleteData: %+v", err)
	}

	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after DeleteData: %+v", err)
	}
