...
func (c NetworkcloudsClient) BareMetalMachinesUncordon(ctx context.Context, id BareMetalMachineId) (result BareMetalMachinesUncordonOperationResponse, err error) {
	opts := client.RequestOptions{
		...
		Path:       fmt.Sprintf("%s/uncordon", id.ID()),
	}
...
func (c NetworkcloudsClient) BareMetalMachinesUncordonThenPoll(ctx context.Context, id BareMetalMachineId) error {
	...
	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after BareMetalMachinesUncordon: %+v", err)
	}
