...
func (c NetworkcloudsClient) CloudServicesNetworksUpdate(ctx context.Context, id CloudServicesNetworkId, input CloudServicesNetworkPatchParameters) (result CloudServicesNetworksUpdateOperationResponse, err error) {
...
func (c NetworkcloudsClient) CloudServicesNetworksUpdateThenPoll(ctx context.Context, id CloudServicesNetworkId, input CloudServicesNetworkPatchParameters) error {
	result, err := c.CloudServicesNetworksUpdate(ctx, id, input)
	if err != nil {
		return fmt.Errorf("performing CloudServicesNetworksUpdate: %+v", err)
	}

	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after CloudServicesNetworksUpdate: %+v", err)
	}
