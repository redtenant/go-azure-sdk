...
func (c VMwaresClient) WorkloadNetworksDeleteDnsServiceThenPoll(ctx context.Context, id DnsServiceId) error {
	result, err := c.WorkloadNetworksDeleteDnsService(ctx, id)
	if err != nil {
		return fmt.Errorf("performing WorkloadNetworksDeleteDnsService: %+v", err)
	}

	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after WorkloadNetworksDeleteDnsService: %+v", err)
	}
