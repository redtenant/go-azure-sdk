...
func (c VMwaresClient) WorkloadNetworksDeleteDnsZoneThenPoll(ctx context.Context, id DnsZoneId) error {
	result, err := c.WorkloadNetworksDeleteDnsZone(ctx, id)
	if err != nil {
		return fmt.Errorf("performing WorkloadNetworksDeleteDnsZone: %+v", err)
	}

	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after WorkloadNetworksDeleteDnsZone: %+v", err)
	}
