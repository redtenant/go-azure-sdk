...
type ListOperationResponse struct {
	...
	OData        *odata.OData
	Model        *[]WorkloadNetwork
}

type ListCompleteResult struct {
	LatestHttpResponse *http.Response
	Items              []WorkloadNetwork
}
...
func (c WorkloadNetworksClient) List(ctx context.Context, id PrivateCloudId) (result ListOperationResponse, err error) {
	opts := client.RequestOptions{
		...
		Path:       fmt.Sprintf("%s/workloadNetworks", id.ID()),
	}
	...
	var values struct {
		Values *[]WorkloadNetwork `json:"value"`
	}
...
func (c WorkloadNetworksClient) ListComplete(ctx context.Context, id PrivateCloudId) (ListCompleteResult, error) {
	return c.ListCompleteMatchingPredicate(ctx, id, WorkloadNetworkOperationPredicate{})
}
