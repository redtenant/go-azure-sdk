...
func (c WorkloadNetworkSegmentsClient) WorkloadNetworksCreateSegments(ctx context.Context, id SegmentId, input WorkloadNetworkSegment) (result WorkloadNetworksCreateSegmentsOperationResponse, err error) {
...
func (c WorkloadNetworkSegmentsClient) WorkloadNetworksCreateSegmentsThenPoll(ctx context.Context, id SegmentId, input WorkloadNetworkSegment) error {
	result, err := c.WorkloadNetworksCreateSegments(ctx, id, input)
	if err != nil {
		return fmt.Errorf("performing WorkloadNetworksCreateSegments: %+v", err)
	}

	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after WorkloadNetworksCreateSegments: %+v", err)
	}
