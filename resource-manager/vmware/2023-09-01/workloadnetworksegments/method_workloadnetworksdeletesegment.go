...
func (c WorkloadNetworkSegmentsClient) WorkloadNetworksDeleteSegmentThenPoll(ctx context.Context, id SegmentId) error {
	result, err := c.WorkloadNetworksDeleteSegment(ctx, id)
	if err != nil {
		return fmt.Errorf("performing WorkloadNetworksDeleteSegment: %+v", err)
	}

	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after WorkloadNetworksDeleteSegment: %+v", err)
	}
